"""CREDIT-ANCHOR-FRONTIER-1 birth driver (pre-reg RESULTS L69122, sealed by
AMENDMENT -PRECISION L69223). Sibling of the results-cited WRITER-DFA-1
driver scratch/birth19m_dfa.py (frozen; its bp and dfa branches survive
here verbatim) which is itself the stock ATOM-DIET-TRAJECTORY-1 recipe.
The credit writer is the only scientific switch:
...
CELL = f"{PHASE}_{MODE}" + (f"_k{K_BP}" if MODE in ("hybrid", "zero") else "") + f"_s{SEED}" + (f"_S{S:g}" if MODE in ("dfa", "hybrid") else "") + f"_lr{PEAK_LR:g}" + TAG
ROOT = Path("checkpoints/writercaf1_smoke" if SMOKE else "checkpoints/writercaf1")
OUTDIR = ROOT / CELL
RECEIPTS = Path("logs/writercaf1/smoke.jsonl" if SMOKE else ("logs/writercaf1/qual.jsonl" if QUAL else "logs/writercaf1/births.jsonl"))
...
def main():
    ...
    enc = enc_stock
    print(f"[caf] mode={MODE} k_bp={K_BP} seed={SEED} cell={CELL}: stock {len(enc_stock)} seq", flush=True)
    ...
    print(f"[caf] stream digests verified stock e0..e2", flush=True)
    if DRYRUN:
    ...
    if MODE == "zero":
        ...
        print(f"[caf] zero-credit: {len(frozen_names)} frozen tensors, {len(trainable_names)} trainable", flush=True)
    ...
    print(f"[caf] steps_total {steps_total} "
    ...
    if MODE in ("dfa", "hybrid"):
        ...
        print(f"[caf] feedback digest {feedback['digest'][:16]} s={S:g}", flush=True)
    ...
    for ep in range(EPOCHS):
        stream = streams[ep]
        print(f"[caf] ep{ep}: {len(stream)} batches", flush=True)
        for a, b in stream:
            batch = enc[a:b]
            ...
            if not torch.isfinite(loss.detach()):
                if QUAL:
                    nonfinite_step = step
                    print(f"[caf] UNSTABLE: non-finite loss at step {step}", flush=True)
                    done = True
                    break
    ...
    row = {"kind": "birth", "prereg": "CREDIT-ANCHOR-FRONTIER-1", "phase": PHASE, "mode": MODE, "k_bp": K_BP, "seed": SEED, "cell": CELL,
    ...
    if nonfinite_step is not None:
        ...
        print(f"[caf] receipt appended to {RECEIPTS} (UNSTABLE at step {nonfinite_step}; no final checkpoint)", flush=True)
        return
    ...
    if MODE == "zero":
        ...
        if w0_back is not None:
            ...
            print(f"[caf] zero-credit freeze law verified: {len(frozen_names)} tensors bit-identical to W_0", flush=True)
    ...
    print(f"[caf] saved {final_p} after {step} steps ({wall:.0f}s)", flush=True)
    ...
    write_receipt(row, launch_commit)
    print(f"[caf] receipt appended to {RECEIPTS} (final state_digest {final_rec['state_digest'][:16]})", flush=True)
