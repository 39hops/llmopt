"""EX6-DEPTH-1 driver (frozen pre-launch): z1 single-block masks
per PRE-REG EX6-DEPTH-1 — does the late band's +19 localize to
the routing decisions at blocks 43 and 46?

Arms (mask predicate over (phase, band position)):
  NONE      nothing masked (anchor: 64/61/66)
  Z1_LATE   z1 masked at MoE blocks 32-47 (anchor: 70/69/71,
            the booked EX6-DEPTH-0 late cells)
  Z1_L32_39 z1 masked at blocks 32-39 — INERTNESS-CHECK cell,
            seed 7001 only: the desk derivation predicts exactly
            the NONE cell (64) with masked recall 1.0
  Z1_B43    z1 masked at block 43 only
  Z1_B46    z1 masked at block 46 only
  Z1_B43_46 z1 masked at blocks 43 and 46
...
Single-block arms are one temporal position x 1-2 layers; realized
displacement DISCLOSED via masked_recall (the -SCOPE lesson: call
dose is not displacement; per the desk census, native z1 outside
demand at 43/46 is ~1 slot per token each).
...
Stages, fail-closed in order:
  census     problem idx 2, seed 7001, one short pass PER ARM
             ...
             exactly (0 for NONE, the arm's named blocks
             otherwise) at phase z1 and nowhere else.
  inert      seed 7001 Z1_L32_39 cell — must read gate 64
             exactly with masked recall 1.0 (the lemma check);
             always-readable (its expected value is the booked
             NONE cell). Miss exits 3.
  qual       per seed: NONE, Z1_LATE — all 6 cells CELL-EXACT
             against the EX6-DEPTH-0 receipts or exit 3 before
             any single-block cell runs or prints.
  treatment  per seed: Z1_B43, Z1_B46, Z1_B43_46
             (sealed-until-qualification receipts + sealed
             stdout redirect).

Receipts under logs/ex6depth1/ (refuse-if-exists; SMOKE=1 ->
smoke_* paths, 4 problems, seed 7001, anchors not enforced):
  census.json, qual.jsonl, inert.jsonl, qual_perprob.jsonl
                                                   ALWAYS-READABLE
...
DIR = Path("logs/ex6depth1")
CENSUS = DIR / f"{PRE}census.json"
INERT = DIR / f"{PRE}inert.jsonl"
...
N_MOE = 48
# Anchors: NONE from logs/ex6depth/qual.jsonl, Z1_LATE from
# logs/ex6depth/treatment.jsonl (booked EX6-DEPTH-0 cells).
ANCHORS = {"NONE": {7001: 64, 8002: 61, 9003: 66},
           "Z1_LATE": {7001: 70, 8002: 69, 9003: 71}}
INERT_EXPECT = 64  # the booked NONE seed-7001 cell
BANDS = {"Z1_LATE": range(32, 48), "Z1_L32_39": range(32, 40),
         "Z1_B43": (43,), "Z1_B46": (46,),
         "Z1_B43_46": (43, 46)}
...
def census_verdict(census, n_moe, arm):
    ...
    want_band = set() if arm == "NONE" else set(BANDS[arm])
    per_module = {}
...
def main():
    for pth in (CENSUS, INERT, QUAL, TREAT, QUAL_PP, TREAT_PP,
                SEALED_OUT):
        if pth.exists():
            raise SystemExit(f"REFUSING: {pth} exists")
    DIR.mkdir(parents=True, exist_ok=True)
    START = start_provenance(
        ["scratch/ex6depth1.py", "scratch/moe_gt1_arm2.py", KEEPSET])
    ...
    try:
        for arm in ("NONE", "Z1_LATE", "Z1_L32_39", "Z1_B43",
                    "Z1_B46", "Z1_B43_46"):
            state, restore = instrument(model, keep, pred_for(arm))
    ...
    # Stage 2: inertness-lemma check — seed 7001 Z1_L32_39, one
    # cell, always-readable (its derived expectation is the booked
    # NONE cell, so nothing treatment-blind is revealed).
    m.SEED = 7001
    problems = make_dataset(N_EVAL, seed=7001)
    n_ok, state = run_arm(model, tok, problems, keep, 7001,
                          "Z1_L32_39", INERT)
    recall = (round(state["hits"] / state["slots"], 4)
              if state["slots"] else None)
    print(f"[dep] inertness Z1_L32_39 seed 7001: {n_ok}/"
          f"{len(problems)} recall {recall}", flush=True)
    if not SMOKE and (n_ok != INERT_EXPECT or recall != 1.0):
        print("[dep] INERTNESS-CHECK FAIL — lemma violated, no "
              "anchor or single-block cell runs", flush=True)
        sys.exit(3)

    # Stage 3: anchors (always-readable).
    miss = []
    for seed in SEEDS:
        problems = make_dataset(N_EVAL, seed=seed)
        m.SEED = seed
        for arm in ("NONE", "Z1_LATE"):
    ...
    with QUAL.open("a") as f:
        f.write(json.dumps({"meta": {
            "note": "EX6-DEPTH-1 qual meta",
            "anchor_misses": miss, "start": START,
            "completion_commit": completion_commit()}}) + "\n")
    if miss:
        print(f"[dep] QUALIFICATION FAIL {len(miss)} anchor cell(s)"
              " — single-block arms do not run", flush=True)
        sys.exit(3)

    # Stage 4: single-block treatment (sealed receipts + stdout).
    m.PERPROB_LOG = TREAT_PP
    for seed in SEEDS:
        problems = make_dataset(N_EVAL, seed=seed)
        m.SEED = seed
        for arm in ("Z1_B43", "Z1_B46", "Z1_B43_46"):
    ...
    with TREAT.open("a") as f:
        f.write(json.dumps({"meta": {
            "note": "EX6-DEPTH-1 run meta", "start": START,
            "completion_commit": completion_commit()}}) + "\n")
