"""MATH-CYBER-1 REGRET-WALLLIFT-0 (PRE-REG booked at d30e84f4).
Closure re-read of the three REGRET-LDS-DESK-0 wall-censored
roots under a lifted 3600 s emergency wall, with world
successor-materialization time and theta_0 scoring time
accumulated SEPARATELY (their sum is the charged wall the kill
checks). Every search law preserved from the parent
(mathworld1_regret.py):
...
sum check, emergency-kill via injected charge. Real mode needs
MW1_WALLLIFT_GO=1.

    SMOKE=1 .venv/bin/python scratch/mathworld1_regret_walllift.py  (Mac)
"""
import hashlib
import heapq
import json
...
if not SMOKE and os.environ.get("MW1_WALLLIFT_GO") != "1":
    raise SystemExit("REFUSING: real mode needs MW1_WALLLIFT_GO=1;"
                     " SMOKE=1 for qualification")
PRE = (f"smoke{os.environ.get('SMOKE_TAG', '')}_"
       if SMOKE else "")
CKPT = Path("checkpoints/mathnative_19m_mw1_theta0.pt")
ROWS = Path(f"logs/mathworld1/{PRE}regret_walllift.jsonl")
VERDICT = Path(
    f"logs/mathworld1/{PRE}regret_walllift_verdict.json")
MAX_DEPTH = 12
EXPANSION_CAP = 96
LADDER = (12, 24, 48, 96)
WALL_CAP_S = 3600.0
...
POP = [("L6-s9108", 6, 9108), ("L4-s9401", 4, 9401),
       ("L4-s9504", 4, 9504)]
...
def regret_search(root, world, scorer, sink, eid,
    ...
    world_s = inject_delay_s      # successor materialization
    model_s = 0.0                 # candidate encoding + forwards
    paid = set()
    table = {}   # key -> node dict
    frontier = []
...
def main():
    ...
    plan = (POP if not SMOKE else
            [("L4-s9100", 4, 9100), ("L7-s9303", 7, 9303)])
    results = {}
    with ROWS.open("a") as f:
        ...
        if SMOKE:
            cens = regret_search(root(4, 9100), world, scorer,
                                 f, "SYNTH-censor",
                                 inject_delay_s=3601.0)
            rows_now = [json.loads(l) for l in ROWS.open()
                        if l.strip()]
            eps_now = [r for r in rows_now
            ...
            checks = {
                ...
                "split_sums_to_charged": all(
                    abs(r["world_materialization_s"]
                        + r["model_scoring_s"]
                        - r["charged_wall_s"]) < 0.005
                    for r in eps_now),
                "both_split_fields_present": all(
                    "world_materialization_s" in r
                    and "model_scoring_s" in r
                    for r in eps_now),
                "emergency_kill":
                    cens["outcome"] == "CENSORED_WALL",
                "ladder_accounting": all(
