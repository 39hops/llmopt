"""MATH-CYBER-1 SVP-GRID-BIRTH-16001 — the FIRST production birth
of the revised IN-SUPPORT-CONFIRMATORY protocol (not a
replication claim): the frozen PAIRED
...
vocab-340 init (seed 16001). ZERO evaluation scoring; this file
...
(SVPGB16_MAKE_INIT=1, which builds+gates+writes the init and
...
Paths (refuse-if-exists in production):
  checkpoints/svp_grid_init_s16001.pt   (pre-step, sha-gated)
  checkpoints/svp_grid_factor_s16001.pt
  checkpoints/svp_grid_hash_s16001.pt
  logs/mathworld1/svpgbirth_s16001_receipt.json
Smoke (isolated): logs/mathworld1/smoke_svpgbirth16.json — both
...
init and creates nothing — it writes only its own receipt.
    ...
    SVPGB16_MAKE_INIT=1 .venv/bin/python scratch/mathworld1_svpgbirth16.py
    .venv/bin/python scratch/mathworld1_svpgbirth16.py            (smoke)
    SVPGB16_PRODUCTION=1 .venv/bin/python scratch/mathworld1_svpgbirth16.py
"""
import hashlib
import io
import json
...
INIT_CK = Path("checkpoints/svp_grid_init_s16001.pt")
# pinned at design time by two independent byte-equal builds,
# 2026-08-29:
INIT_SHA = ("2a580568e37ec91d976dbde0d4654a371f8a31f66dd514a7"
            "8bdfd1afed3dbbaf")
CKS = {"FACTOR": Path("checkpoints/svp_grid_factor_s16001.pt"),
       "HASH": Path("checkpoints/svp_grid_hash_s16001.pt")}
RECEIPT = Path("logs/mathworld1/svpgbirth_s16001_receipt.json")
SMOKE_RECEIPT = Path("logs/mathworld1/smoke_svpgbirth16.json")
MAKE_INIT = os.environ.get("SVPGB16_MAKE_INIT") == "1"
TOK = ActionGCTok()
PRODUCTION = os.environ.get("SVPGB16_PRODUCTION") == "1"
...
def make_init():
    ...
    print(f"[svpgb16] init written {INIT_SHA}", flush=True)


def ensure_init():
    gate(INIT_CK.exists(),
         "INIT ABSENT (run SVPGB16_MAKE_INIT=1 pre-step)")
...
def run(plan, rows_by_id, dev, init_sha, n_steps, tag):
    ...
    for step, (e, bi, ids_) in enumerate(plan[:n_steps]):
        ...
        if step % 200 == 0 or step == n_steps - 1:
            print(f"[svpgb16:{tag}] step {step}/{n_steps} " +
