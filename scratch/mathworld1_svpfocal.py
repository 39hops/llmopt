"""MATH-CYBER-1 SVP-FIELD-ORDER-CALIBRATION-SCORE-19001 — ONE
scoring run of the two sealed seed-19001 field-order checkpoints
(CANONICAL / PARAM-FIRST) on the frozen covered-calibration
artifact ONLY (af1a4aa1..., the same 96 covered-I0 strict
states). Stage 2 of the frozen FIELD-ORDER protocol (prereg
3ac5a70e). No heldout scoring, no token-onset values, no
...
PERM law), gated per candidate by pf_decode roundtrip AND the
...
Outputs under logs/mathworld1/svpfocal/ (refuse-if-exists):
scores.jsonl, svpfocal_receipt.json, riders.json.

    .venv/bin/python scratch/mathworld1_svpfocal.py          (Mac)
"""
import hashlib
import json
import sys
...
PINS = {
    ...
    "logs/mathworld1/svpforder_s19001_receipt.json":
        "f9565f745cfe92462c5f35e938d2a3e6d50e646533e16363e09f"
        "884190856b25",
    "logs/mathworld1/svpforder_census/"
    "svpforder_census_receipt.json":
        "3f65b7ea3f4c92f241e127323fc3aa1ed5fb4879ac6741178baf"
        "87bc3817fc96",
}
CKPTS = {
    "CANONICAL": ("checkpoints/svp_forder_canonical_s19001.pt",
                  "ae0a86e027d8b0ca1cd7a97a83a6927d326da5bd34"
                  "258910b1b81d3492322e1d"),
    "PARAM_FIRST": ("checkpoints/svp_forder_paramfirst_s19001"
                    ".pt",
                    "0fe38f785f68165e868c54fff482844ea4b2476c"
                    "737f2e4af50990ece6df390f"),
}
INIT_SHA = ("20751529f22e3f5da4bfdf1504fbc8e8f33200003de2eec7"
            "848a2a146120339e")
INIT_CK = "checkpoints/svp_forder_init_s19001.pt"
VOCAB = 340
CODE_BASE = 332
ARMS = ["CANONICAL", "PARAM_FIRST"]
BAR_REGIME = 36
BAR_STRATUM = 12
OUTDIR = Path("logs/mathworld1/svpfocal")
TOK = ActionGCTok()
...
def main():
    ...
    verdict = ("FIELD-ORDER CALIBRATION FIRED" if fired
               else "FIELD-ORDER-NOT-REACHED")
    receipt = {
        "prereg": "MATH-CYBER-1-SVP-FIELD-ORDER-CALIBRATION-"
                  "SCORE-19001",
        "verdict": verdict,
    ...
    (OUTDIR / "svpfocal_receipt.json").write_text(
    ...
    print("[svpfocal] DONE", flush=True)
    return 0
