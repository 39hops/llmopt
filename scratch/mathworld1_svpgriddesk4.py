...
SECOND family — the two-pair construction (U-F2 lineage) ported
to the small non-integrable distractors — so the frozen >= 2
family clause becomes satisfiable. Verdict computed over the
UNION of SPACE-3 and SPACE-4 successes (both spaces frozen
pre-run; union law stated here before this space runs).
...
FROZEN SPACE-4 (deterministic, each grid point once):
  U-F4: state = Integral(f_t, x) + Integral(D, x), where
    f_t = expand(d/dx[P1*sin(2*x)]) + expand(d/dx[P2*cos(3*x)])
          + Integral(w, x)
    (P1,P2) in {(x^3,x), (x^2+x,x), (x^3,x^2), (x^4,x),
    (x^2,x^3)}; w in {exp(x)/x, sin(x)/x};
    D in {x**x, 1/(x+log(x)), sin(sin(x))}.
    Grid: 5*2*3 = 30 attempts.
  Site ordinals/term indices MEASURED, never assumed.

Outputs logs/mathworld1/svpgriddesk4_receipt.json
(refuse-if-exists).

    .venv/bin/python scratch/mathworld1_svpgriddesk4.py       (Mac)
"""
import json
import sys
...
from scratch.mathworld1_svpgriddesk import (COVERED_P,  # noqa: E402
                                            ...
                                            PAIRED_SHA, X, ckey,
                                            fsha, run_attempt)
RECEIPT = Path("logs/mathworld1/svpgriddesk4_receipt.json")
PRIOR = Path("logs/mathworld1/svpgriddesk3_receipt.json")
        ...
        for w in (sp.exp(X) / X, sp.sin(X) / X):
            for D in DISTRACTORS:
                f = (sp.expand(sp.diff(P1 * sp.sin(2 * X), X))
                     ...
                     + sp.Integral(w, X))
                space.append(("U-F4", "i_unprod",
                              f"P1={P1} P2={P2} w={w} D={D}",
                              f, D))
    return space
...
def main():
    ...
    for pr in prior["attempts"]:
        if pr.get("success"):
            cell = tuple(pr["chosen_cell"].split())
            cell = (cell[0], cell[1], int(cell[2]), cell[3],
                    int(cell[4]))
            succ[cell].add(pr["parent_sstr"])
            succ_fams[cell].add(pr["family"])
            fam_cells[pr["family"]].add(cell)
    intents = Counter()
    outcomes = Counter()
    setsizes = []
    comp = Counter()
    ...
    receipt = {
        "space": "SPACE-4 (verdict over SPACE-3 union SPACE-4)",
        "verdict": verdict,
        "cell_summary": {
