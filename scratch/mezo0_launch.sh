...
# logs/mezo0/desk.log (refuses if it exists); writes logs/mezo0.DONE on
...
mkdir -p logs/mezo0
nohup bash -c '.venv/bin/python scripts/liverun.py run mezo0 -- .venv/bin/python scratch/mezo_signal_desk.py && echo DONE > logs/mezo0.DONE' > logs/mezo0/desk.log 2>&1 &
echo "launched pid $!"
