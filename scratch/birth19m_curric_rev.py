"""REV-LADDER-1 instrument run (pre-reg RESULTS 2026-08-14): the
CURRICULUM-1 cap arm with the ladder REVERSED — hard-first
L8,L4,L6,L7,L5,L3,L2,L1. Thin sibling of the frozen
...
noop_rev.log). Same BIRTH_SEED=2, 15,420 steps, stock OneCycle,
mps; level-pure batches, within-level shuffled, stream truncated
to the stock per-epoch count (truncation falls on L1's tail,
~0.3% of that level). Appends arm="rev" to logs/curric1/rev.jsonl (arms.jsonl is
a frozen receipt).
...
OUT = Path("checkpoints/gallery19m_curric_rev_s2.pt")
if OUT.exists():
...
REV_LADDER = [8, 4, 6, 7, 5, 3, 2, 1]
EPOCHS, BS = C.EPOCHS, C.BS
RECEIPTS = Path("logs/curric1/rev.jsonl")
...
def main():
    ...
    print(f"[rev] {len(enc)} sequences, vocab {len(tok.vocab)}",
          flush=True)
    ...
    print(f"[rev] steps_total {steps_total} ({steps_per_epoch}/epoch)",
          flush=True)

    step = 0
    t0 = time.time()
    for ep in range(EPOCHS):
        lb = C.level_batches(levels, f"curric-rev-{ep}")
        stream = [b for l in REV_LADDER for b in lb.get(l, [])]
        dropped = len(stream) - steps_per_epoch
        stream = stream[:steps_per_epoch]
        print(f"[rev] ep{ep}: {len(stream)} batches ({dropped} tail "
    ...
    print(f"[rev] saved {OUT} after {step} steps "
