"""MATH-CYBER-1 SVP-FIELD-ORDER-REPLICATION-MATERIALIZATION-
INTEGRITY-BIRTH-20001 — stage 1 of the frozen FIELD-ORDER
REPLICATION protocol (replication prereg 732a5312, adopting
world + laws byte-for-byte from prereg 3ac5a70e). INIT-ONLY
replication: seed 20001 is the SOLE changed scientific
variable; every training-law line is carried over from the
booked seed-19001 instrument scratch/mathworld1_svpforder.py
unchanged. This file re-runs qualification, the
HARD-STOP pre-training structural census, the descriptive
exposure census, and (only when every gate has passed) the
two-arm seed-20001 BIRTH. No calibration, no heldout scoring,
...
Paths (refuse-if-exists in production; ALL disjoint from every
seed-19001 artifact — no 19001 path is reused):
  checkpoints/svp_forder_init_s20001.pt   (pre-step, sha-gated)
...
Census: logs/mathworld1/svpforepl_census/ (refuse-if-exists)
Smoke: logs/mathworld1/smoke_svpforepl20.json

    SVPFO_CENSUS=1 .venv/bin/python scratch/mathworld1_svpforepl.py
    SVPFO_MAKE_INIT=1 .venv/bin/python scratch/mathworld1_svpforepl.py
    .venv/bin/python scratch/mathworld1_svpforepl.py         (smoke)
    SVPFO_PRODUCTION=1 .venv/bin/python scratch/mathworld1_svpforepl.py
"""
import hashlib
import io
import json
...
INIT_CK = Path("checkpoints/svp_forder_init_s20001.pt")
# pinned at this stage's commit by two independent byte-equal
# builds, 2026-08-30:
INIT_SHA = ("7c95e77f8d7ccea5f4dd71c989e4d3225e347a178032d435"
            "39a1ae6ef62c9452")
CKS = {"CANONICAL":
       Path("checkpoints/svp_forder_canonical_s20001.pt"),
       "PARAM_FIRST":
       Path("checkpoints/svp_forder_paramfirst_s20001.pt")}
RECEIPT = Path("logs/mathworld1/svpforepl_s20001_receipt.json")
SMOKE_RECEIPT = Path("logs/mathworld1/smoke_svpforepl20.json")
CENSUS_DIR = Path("logs/mathworld1/svpforepl_census")
CENSUS_RECEIPT = CENSUS_DIR / "svpforepl_census_receipt.json"
...
def run_census():
    ...
    receipt = {
        "prereg": "MATH-CYBER-1-SVP-FIELD-ORDER-REPLICATION-"
                  "PREREG-0",
        "replication_of": "seed 19001 (prereg 3ac5a70e)",
        "stage": "replication materialization-integrity/census",
    ...
    print("[svpforepl:census] DONE", flush=True)
...
def make_init():
    ...
    print(f"[svpforepl] init written {INIT_SHA}", flush=True)
...
def run(plan, rows_by_id, dev, init_sha, n_steps, tag):
    ...
    for step, (e, bi, ids_) in enumerate(plan[:n_steps]):
        ...
        if step % 200 == 0 or step == n_steps - 1:
            print(f"[svpforepl:{tag}] step {step}/{n_steps} " +
...
def main():
    ...
    print("[svpforepl] DONE", flush=True)
    return 0
