"""MATH-CYBER-1 SVP-GRID-BIRTH-17001 — the seed-17001 INIT-ONLY
replication birth under the frozen law of
REPLICATION-17001-PREREG-0 (cd6db7e3): the frozen PAIRED
...
vocab-340 init (seed 17001). ZERO evaluation scoring; this file
has no path or string dependency on any evaluation artifact —
the frozen IN-SUPPORT eval shas (calibration / sealed heldout /
secondary robustness) appear ONLY as opaque provenance literals
copied into the receipt (EVAL_PROVENANCE below), never as
paths, and nothing here opens, parses, counts, or scores them.
Successor fix folded (BIRTH-15001 disclosed deviation): the
shared init is materialized by a DEDICATED PRE-STEP
(SVPGB17_MAKE_INIT=1, which builds+gates+writes the init and
exits); smoke and production REFUSE if the init is absent, so
smoke never writes any production path.
...
Paths (refuse-if-exists in production):
  checkpoints/svp_grid_init_s17001.pt   (pre-step, sha-gated)
  checkpoints/svp_grid_factor_s17001.pt
  checkpoints/svp_grid_hash_s17001.pt
  logs/mathworld1/svpgbirth_s17001_receipt.json
Smoke (isolated): logs/mathworld1/smoke_svpgbirth17.json — both
...
scheduler steps, both arm orders; smoke REQUIRES the pre-step
init and creates nothing — it writes only its own receipt.

    SVPGB17_MAKE_INIT=1 .venv/bin/python scratch/mathworld1_svpgbirth17.py
    .venv/bin/python scratch/mathworld1_svpgbirth17.py            (smoke)
    SVPGB17_PRODUCTION=1 .venv/bin/python scratch/mathworld1_svpgbirth17.py
"""
import hashlib
import io
import json
...
EVAL_PROVENANCE = {
    ...
    "secondary_robustness_sha": (
        "5c85fc1f336791522db78e681bfdadad8c4efdaafed640a7aa50"
        "3d72a82c6137"),
    "eval_blocks_sha": (
        "d500ee554e1cbfe6cb9e8e594b8324f9529f08710e3312d1375a"
        "4f44a4c5ca0b"),
    "pout_attempts_sha": (
        "3f63680ab29724644b9c564cd31f71faf6a78b602e01b102774b"
        "6520a8097dc8"),
    "envelope_receipt_sha": (
...
INIT_CK = Path("checkpoints/svp_grid_init_s17001.pt")
# pinned at design time by two independent byte-equal builds,
# 2026-08-29:
INIT_SHA = ("4384ed9800962f1af24a87e5901a1e348ff982532ac8250c"
            "1aafab983abbbc1a")
CKS = {"FACTOR": Path("checkpoints/svp_grid_factor_s17001.pt"),
       "HASH": Path("checkpoints/svp_grid_hash_s17001.pt")}
RECEIPT = Path("logs/mathworld1/svpgbirth_s17001_receipt.json")
SMOKE_RECEIPT = Path("logs/mathworld1/smoke_svpgbirth17.json")
MAKE_INIT = os.environ.get("SVPGB17_MAKE_INIT") == "1"
TOK = ActionGCTok()
PRODUCTION = os.environ.get("SVPGB17_PRODUCTION") == "1"
...
def make_init():
    ...
    print(f"[svpgb17] init written {INIT_SHA}", flush=True)


def ensure_init():
    gate(INIT_CK.exists(),
         "INIT ABSENT (run SVPGB17_MAKE_INIT=1 pre-step)")
...
def run(plan, rows_by_id, dev, init_sha, n_steps, tag):
    ...
    for step, (e, bi, ids_) in enumerate(plan[:n_steps]):
        ...
        if step % 200 == 0 or step == n_steps - 1:
            print(f"[svpgb17:{tag}] step {step}/{n_steps} " +
