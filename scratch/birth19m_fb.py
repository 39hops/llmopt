"""FROZEN-BACKBONE-1 birth driver. Sibling of the results-cited
CREDIT-ANCHOR-FRONTIER-1 driver scratch/birth19m_caf.py (frozen; every
branch survives here verbatim; the DFA branches are never selected under
the FB law). Two arms per fresh paired seed, same byte-identical W_0 and
the exact stock stream:
  FULL   MODE=bp: ordinary backprop over all parameters (the stock recipe).
  FROZEN MODE=zero K_BP=4: emb + blocks 0..3 frozen exactly at W_0,
         blocks 4..7 + norm + head trained by ordinary backprop
         (dfa_credit.freeze_lower; the freeze law re-verified at the end).
FB=1 seeds 24, 25, 26 only, LR 3e-4, 15,420 steps, 17 snapshots; paths
checkpoints/frozenbb1/ and logs/frozenbb1/births.jsonl. SMOKE=1: seed 11,
checkpoints/frozenbb1_smoke/, logs/frozenbb1/smoke.jsonl. Everything else
(QUAL / DISCOVERY branches, hybrid / dfa modes) is inherited and refused
under FB=1.

Inherited credit-switch documentation:
  ...
  MODE=hybrid K_BP=k  blocks 8-k..7 + norm + head one attached backprop
            segment, x_{8-k} detached at the boundary, blocks 0..7-k the
            DFA law above (dfa_credit.hybrid_objective).
  MODE=zero K_BP=k    zero-credit control: blocks 0..7-k and emb frozen
...
assert MODE in ("bp", "dfa", "hybrid", "zero"), MODE
K_BP = int(os.environ["K_BP"]) if MODE in ("hybrid", "zero") else (0 if MODE == "dfa" else 8)
assert 0 <= K_BP <= 8, K_BP
QUAL = os.environ.get("QUAL") == "1"
DISCOVERY = os.environ.get("DISCOVERY") == "1"
SMOKE = os.environ.get("SMOKE") == "1"
FB = os.environ.get("FB") == "1"
assert QUAL + DISCOVERY + SMOKE + FB == 1, "exactly one of QUAL / DISCOVERY / SMOKE / FB"
FB_SEEDS = (24, 25, 26)
FB_K = 4
...
QUAL_CELLS = [(1.0, 3e-4), (1.0, 1e-4)]
QUAL_K = (1, 2, 4)
ZERO_LR = 3e-4
SELECTION = Path("logs/writercaf1/qual_selection.json")
if SMOKE:
    assert SEED == SMOKE_SEED, f"smoke seed is {SMOKE_SEED}, got {SEED}"
    if MODE in ("dfa", "hybrid"):
        assert S > 0, "S required in dfa / hybrid mode"
    if MODE == "zero":
        assert PEAK_LR == ZERO_LR, "zero-credit control runs at LR 3e-4"
    # smoke admits every K_BP in 0..8 (0 and 8 are the endpoint identities of F1); QUAL admits QUAL_K only
elif FB:
    assert EMIT and not DEVICE_OVERRIDE and not TAG and not GRADDUMP, "EMIT/DEVICE/TAG/GRADDUMP are smoke-only"
    assert SEED in FB_SEEDS, f"FROZEN-BACKBONE-1 seeds are {FB_SEEDS}, got {SEED}"
    assert MODE in ("bp", "zero"), "FROZEN-BACKBONE-1 arms are MODE=bp (FULL) and MODE=zero K_BP=4 (FROZEN)"
    assert PEAK_LR == 3e-4, "FROZEN-BACKBONE-1 runs the stock LR 3e-4"
    if MODE == "zero":
        assert K_BP == FB_K, "FROZEN = emb + blocks 0..3 frozen (K_BP=4)"
else:
    assert EMIT and not DEVICE_OVERRIDE and not TAG and not GRADDUMP, "EMIT/DEVICE/TAG/GRADDUMP are smoke-only"
    if QUAL:
        assert SEED == QUAL_SEED and MODE in ("hybrid", "zero"), "qualification is MODE=hybrid or MODE=zero at seed 23 only"
        assert K_BP in QUAL_K, f"K_BP must be one of {QUAL_K}"
        if MODE == "hybrid":
        ...
        if MODE == "bp":
            ...
            assert MODE == "hybrid", "discovery arms are MODE=bp or MODE=hybrid"
            sel = json.loads(SELECTION.read_text())
            assert sel.get("selected"), "no qualification selection frozen"
            assert (K_BP, S, PEAK_LR) == (int(sel["selected"]["k"]), float(sel["selected"]["s"]), float(sel["selected"]["lr"])), f"(K_BP, S, LR) differ from the frozen selection {sel['selected']}"
...
PHASE = "smoke" if SMOKE else ("fb" if FB else ("qual" if QUAL else "disc"))
CELL = f"{PHASE}_{'full' if (FB and MODE == 'bp') else ('frozen' if (FB and MODE == 'zero') else MODE)}" + (f"_k{K_BP}" if MODE in ("hybrid", "zero") and not FB else "") + f"_s{SEED}" + (f"_S{S:g}" if MODE in ("dfa", "hybrid") else "") + f"_lr{PEAK_LR:g}" + TAG
ROOT = Path("checkpoints/frozenbb1_smoke" if SMOKE else "checkpoints/frozenbb1")
OUTDIR = ROOT / CELL
RECEIPTS = Path("logs/frozenbb1/smoke.jsonl" if SMOKE else "logs/frozenbb1/births.jsonl")
...
def main():
    ...
    enc = enc_stock
    print(f"[fb] mode={MODE} k_bp={K_BP} seed={SEED} cell={CELL}: stock {len(enc_stock)} seq", flush=True)
    ...
    print(f"[fb] stream digests verified stock e0..e2", flush=True)
    if DRYRUN:
    ...
    if MODE == "zero":
        ...
        print(f"[fb] zero-credit: {len(frozen_names)} frozen tensors, {len(trainable_names)} trainable", flush=True)
    opt = torch.optim.AdamW([p for p in model.parameters() if p.requires_grad], lr=PEAK_LR,
                            weight_decay=0.01)
    sched = torch.optim.lr_scheduler.OneCycleLR(
        opt, max_lr=PEAK_LR, total_steps=steps_total, pct_start=0.03)
    print(f"[fb] steps_total {steps_total} "
    ...
    if MODE in ("dfa", "hybrid"):
        ...
        print(f"[fb] feedback digest {feedback['digest'][:16]} s={S:g}", flush=True)
    ...
    for ep in range(EPOCHS):
        stream = streams[ep]
        print(f"[fb] ep{ep}: {len(stream)} batches", flush=True)
        for a, b in stream:
            batch = enc[a:b]
            ...
            if GRADDUMP and step == 0:
                grads = {n: p.grad.detach().to("cpu").clone() for n, p in model.named_parameters() if p.grad is not None}
            opt.step()
            ...
            if GRADDUMP and step == 1:
                torch.save({"grads": grads, "params": {k: v.detach().to("cpu").clone() for k, v in model.state_dict().items()},
                            "loss": float(loss.detach()), "mode": MODE, "k_bp": K_BP, "seed": SEED, "s": S, "lr": PEAK_LR,
                            "batch": [a, b], "lr_at_step1": opt.param_groups[0]["lr"]}, GRADDUMP)
            if not torch.isfinite(loss.detach()):
                if QUAL:
                    nonfinite_step = step
                    print(f"[fb] UNSTABLE: non-finite loss at step {step}", flush=True)
                    done = True
                    break
    ...
    row = {"kind": "birth", "prereg": "FROZEN-BACKBONE-1", "phase": PHASE, "arm": ("FULL" if MODE == "bp" else "FROZEN") if FB else None, "mode": MODE, "k_bp": K_BP, "seed": SEED, "cell": CELL,
    ...
    if nonfinite_step is not None:
        ...
        print(f"[fb] receipt appended to {RECEIPTS} (UNSTABLE at step {nonfinite_step}; no final checkpoint)", flush=True)
        return
    ...
    if MODE == "zero":
        ...
        if w0_back is not None:
            ...
            print(f"[fb] zero-credit freeze law verified: {len(frozen_names)} tensors bit-identical to W_0", flush=True)
    ...
    print(f"[fb] saved {final_p} after {step} steps ({wall:.0f}s)", flush=True)
    ...
    write_receipt(row, launch_commit)
    print(f"[fb] receipt appended to {RECEIPTS} (final state_digest {final_rec['state_digest'][:16]})", flush=True)
