"""GRF-NOTHINK analysis: byte-copy of the frozen grf_analyze.py
with ONLY the three receipt paths changed (PRE-REG GRF-NOTHINK-0
transport discipline; the frozen original is results-cited and
never edited).
...
Receipt: logs/grf/analysis_nothink.json (refuse-if-exists).
...
D = 48 * 128
OUT = Path("logs/grf/analysis_nothink.json")


def main():
    if OUT.exists():
        raise SystemExit(f"REFUSING: {OUT} exists")
    START = start_provenance(["scratch/grf_analyze2.py", "scratch/grf_analyze.py",
                              "scratch/grf_capture.py",
                              "scratch/grf_corpus.py"])
    meta = {}
    for line in open("logs/grf/rows_nothink.jsonl"):
        r = json.loads(line)
        if "pid" in r:
            meta[r["pid"]] = r
