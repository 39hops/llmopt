...
**The record (2026-07-08, ~1 a.m.): markov3 @ width=2 — 316/360 (87.8%).** The width dividend compounded on the bigram dictionary (+23 over its own width-8 run, +16 over the LLM-guided champion's 300): the all-time-best engine on the held-out races is six hand-written rules, a rule-bigram count table, k=3 pruning, and a width-2 beam. Zero neural networks, ~35 CPU-minutes for the full table. Refinement of the day's thesis: the GPU buys confidence, but *width is a partial substitute for confidence* — at width 8 you must know which line to trust; at width 2 the beam hedges just enough that grammar + depth carries it. The LLM's width-2 rematch is queued.
