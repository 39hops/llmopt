...
| `mathgen/` | sympy-verified calculus generator (6 task kinds, difficulty levels, reverse-constructed integrals, symbolic-equivalence checker, contamination-proof train/eval split); LoRA'd Qwen2.5-0.5B: 15.7% → 65.7% symbolic accuracy on unseen problems (`scripts/train_calculus.py`; differentiation 94-100%, integration 79-97%, limits ~15% — multi-step reasoning is the open front) | step-trace training, expert iteration |
| `codegen/` | MSVC compiler-as-oracle (diagnostics, optimized asm, per-instruction byte encodings via dumpbin, program output) | capability-ladder dataset + 0.5B benchmark |
