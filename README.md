...
Markov ambush (`scripts/markov_prior.py`, `scripts/bench_markov.py`): a rule-bigram count table built from the same winning paths — zero training, zero inference cost — ties the fine-tuned 0.5B at top-3 offline (99.7% = 99.7%) and then **beats it in-search: 293/360 vs the LLM's 288**, running the full race in ~35 CPU-minutes vs ~6 GPU/MPS-hours. The proposer's +11-over-random pruning value was almost entirely rule *grammar*; grammar fits in a dictionary. Standing above it: only adaptive-k (300), whose entropy signal still comes from LLM scores — the Markov-confidence variant is the immediate follow-up.
