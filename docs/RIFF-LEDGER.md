...
| CORRECTION (2026-09-08) to the row above | The row states "imaginary-time Schrodinger evolution IS gradient descent" as a theorem with no derivation, and attaches "the law we measured four ways" to the ground-state-degeneracy clause; those four measurements (weight-reader ablation, nf4 distance null, committee lottery redraw, neuron biography) measure GAUGE degeneracy only and say nothing about a Boltzmann distribution, a temperature, an energy or a ground state of training, none of which the house has ever measured (audit 2026-09-08, RIFF bank "A WEIGHT-SPACE HAMILTONIAN WRITER, HONESTLY PRICED"). The precise statements are: SGLD samples the Boltzmann density in the long-time limit; after the standard similarity transform the Fokker-Planck generator of that Langevin dynamics becomes a Schrodinger-form operator whose effective potential contains gradient-norm and Laplacian-of-loss terms, and whose ground state is proportional to sqrt(p_inf) (stochastic quantization); this transformed operator is NOT the loss-potential Hamiltonian H = -a Laplacian + L, whose ground-state density is not in general exp(-L / T); plain gradient descent is the zero-noise Langevin drift, not the imaginary-time evolution of psi. The dictionary stays banked as a FRAME; its "theorem" and "measured" clauses are withdrawn; the THEORY row it seeded ("Training relaxes to a Boltzmann distribution", status "direct application") is flagged as a reframing with no measured anchor (OBSERVATION COUPLED-LEARNING-AND-HAMILTONIAN-WRITER-ASSESSMENT-0). | corrected in place 2026-09-08; frame retained, claims withdrawn |
...
- **BANKED (2026-09-05): DATA -> WEIGHTS -> FUNCTION ON A REAL OPEN CHECKPOINT LIFECYCLE, WITH CROSS-SCALE TRANSPORT AS THE FALSIFIABLE RIDER** (Artin: the program frame, the K2 Horizon candidate, the renormalization / timeframe analogy stated as analogy only, the cross-scale observable list, the contamination law, the marginal-v-context lesson carried from the render-atlas arc; house: artifact verification, ranking, cost ladder, the zero-training first rung).
  ...
  MEASURED (2026-09-06, VERDICT K2-HORIZON-TRANSPORT-0-PROSPECTIVE-7B, RESULTS L66521): residue (2) CLOSED at this rung's ceiling — T1, T2, T3 (both orderings) and T5 fire; the 3.7B lifecycle signatures predicted the 7B ones before 7B was read (depth-profile Spearman 0.915-1.000 on 6/6 pairs, class-share 0.939-1.000 on 6/6, localization orderings 0.771 / 0.943, lag sign 9/9). T4 NOT-RESOLVABLE by law; capability-direction transport stays unmeasured. The renormalization analogy remains an analogy: what transported is a finite description of where and how much a released lifecycle changes the weights at two sizes of one family, not a scale law. The 0.9B chain is a separate lineage and was never compared to either size.
- **BANKED (2026-09-07): ATOM-DIET-TRAJECTORY-1 AND THE DATA x LOCUS INTERACTION TEST THAT FOLLOWS IT** (Artin: the vehicle choice (the booked ATOM-DIET-LADDER-1 intervention instead of a new diet factorial), the no-gates-during-training and counterbalancing laws, the strict geometric resolution law without a pseudo-p-value, the claim-axis separation STRUCTURAL / FUNCTION / JOINT, the canonical state-digest equality law, and the mediation-v-importance design; house: pins, schedule, structural key law, cost).
  The mapping: the K2 Horizon arc (RESULTS L66521) transported an observational lifecycle anatomy across two sizes but could not resolve capability direction and could not arm any causal data intervention (the L9112 residue (3) gate was NOT met: 0.9B stage capability deltas sat inside the resolution floor, and the profile that transported was 3.7B -> 7B, not 0.9B -> 3.7B; residue (3) is therefore NOT armed). The house already holds a randomized DATA intervention with an exact function gate and a replicated paired lift (ATOM-DIET-LADDER-1, +6 / +6 at seeds 3 and 4), so the cheapest causal DATA -> WEIGHTS question is whether that intervention produces a reproducible weight-trajectory anatomy beyond paired birth variation at the house 19M scale. PRE-REG ATOM-DIET-TRAJECTORY-1 (RESULTS L66546) freezes six births, 17 fixed snapshots each, post-hoc gates only after all six trainings, a counterbalanced 2:1 arm order from a fixed hash (atoms-first seed 7), six actual token-stream digests, the literal 59-key P8 / C8 law, the strict geometric S1 law, and the claim axes STRUCTURAL = S1 AND S1b, FUNCTION = F1, JOINT = both.
  Measured anchors: ATOM-DIET-LADDER-1 (L29465) paired +6 / +6, L4 12 / 12; fixed-seed mps run-noise anchor n = 1 pair (booked stock s3 v the L29985 rerun control): P8 L2 0.001283, C8 L2 0.000618, recorded in the pre-reg as context, not a floor; K2 transport bars at L66521 for the shape of the objects, with the 8-layer raw profile replacing K2's 10-bin coordinate.
  Honest breaks: mps fp32 training is run-level nondeterministic (L29985), so within-arm displacements carry substrate noise and arm order is only counterbalanced, never removed, at n = 3; the snapshot-emitting execution is a new regime, so the function lift must reproduce under it (F1) and is not assumed; a firing STRUCTURAL bar is a description of where the diet's update energy lands, not a lever; the 3.5% dose and the one-ply rule are fixed (dose and rule generality belong to the dose and rule-ablation rungs).
  Testable residue: (1) ATOM-DIET-TRAJECTORY-1 itself (L66546); (2) BANKED FOR LATER, not in this pre-reg: if STRUCTURAL fires, the WEIGHTS -> FUNCTION test is a DATA x locus-intervention INTERACTION on fresh seeds 8, 9, 10, never a bare "freeze the high-response band in atom births": (stock, atoms) x (high-response band frozen at init, equal-parameter low-response band frozen, matched-random parameter set frozen), with the mediation reading drawn only if the atom lift shrinks under the high-response freeze by more than under both controls AND the stock arm's own loss under the same freeze is smaller than that shrinkage (the interaction term), which separates mediation of the atom lift from generic importance of the band; partition identities as in bank M; (3) if STRUCTURAL fails with FUNCTION firing, the K2-derived macro-trajectory-signature-as-data-lever program STOPS at this house scale, and only that program.
  Attribution: Artin (the vehicle, the blockers and the interaction design), house (pins, schedule, key law, verifier contract).
- **BANKED (2026-09-07): DECLARATIVE COMPUTE — LANGUAGE AS A CONTROL PLANE (the generalization of Declarative Attention), with the three-way split declaration quality / intervention quality / declaration faithfulness and a zero-training causal factorial** (Artin: the frame, the generalization from KV-cache masks to engine calls / delegation / experts / depth, the three-way split, the five-arm factorial, the trained reward-minus-cost residue held behind the controls; house: ledger anchors, paper read, wording).
  The mapping: a model emits ordinary symbolic tokens; a TRUSTED RUNTIME interprets them as commands that change the substrate available to subsequent computation. Declarative Attention (Ho, Ahmad, Koh, Yun, Schuster, Nogueira dos Santos, arXiv 2609.02737, submitted 2026-09-02, read in full 2026-09-07) is one instance: the model writes `<global>`, `<focus magic_chunks="K">` and `<local>` tags inside its own output, a state machine in the inference engine parses them at the closing `>` and masks whole KV-cache blocks so that focus mode attends only the named context segments and local mode attends none of them; on off-the-shelf models with no fine-tuning, attended tokens fall 52.0% (Gemma-4-31B, 87.01% to 85.74%, a 1.27 pp accuracy drop) and 31.1% (Qwen-3.6-27B, 85.31% to 82.56%, 2.75 pp) across 15 long-context tasks with thinking disabled; the DA-no-mask control (same prompt template, full causal attention) matches vanilla accuracy at Gemma-4-31B (87.01% v 87.01%) while attending 66.2% MORE tokens, so the mask is what converts the declaration overhead into a saving; focus-tag parse success rises from 58% at Gemma-4-E4B to 99% at Gemma-4-31B; the authors report NO oracle-region or random-region control and NO test of whether declared regions are causally necessary, and state that models failed to follow the protocol inside thinking traces. In llmopt the same shape already exists with an EXACT oracle on the callee side: a verified engine call written into the row (the prosthetic), a trie that restricts the legal token substrate, a mask that restricts the expert substrate, and the LLMUE machinery that resolves a call at decode.
  The three axes to keep apart: (1) DECLARATION QUALITY, did the model choose the useful resource; (2) INTERVENTION QUALITY, does supplying that resource or mask improve function; (3) DECLARATION FAITHFULNESS, does the declared resource correspond to what is causally necessary. DA measures (2) only (through DA-no-mask) and leaves (1) and (3) open; the house ledger has strong (2), one head-to-head on (1) that reads model-worse on a two-discordant margin, and NO booked measurement of (3).
  Measured anchors (verified against the cited lines 2026-09-07): (2) STEP-LOCAL CALL-SPAN VERDICT, RESULTS L9998, PLAIN 44/100 v SPAN 59/100, +15 at n = 100 with the end-value paired control +4 sub-sigma (L9967), n = 1 per arm, mps, exact match, the span inserted by the harness and never chosen by the model; (2) MATH-CYBER-1-SVP-DECODE-CONSTRAINT-LADDER, RESULTS L58795, legal-trie greedy lifts strict-heldout free generation from 0 / 96 to 85 / 78 / 75 / 87 of 96 on four checkpoints, a runtime restricting the token substrate, with CHOSE-TEACHER 56 / 49 / 46 / 58 and zero forced, so the model selects among 2 to 3 legal alternatives (partial (1)); (2) MoE crest MOE-GT-1-R5, RESULTS L18927, 45.3% demand-ranked keep beats the paired full model +17 / +9 / +18 at three unspent seeds (pooled +14.7 against a registered +7), and MOE-GT-1-R6, L19108, random-45.3% masks 0 / 120 twice and anti-demand 0 / 120 at the same keep fraction, which is the nearest thing the ledger has to a necessity test of a declared resource, except that the demand ranking is computed offline from routing logs and is never emitted by the model; (1) MATH-CYBER-1-CLOSED-LOOP-1-JOINT-THREE-ARM-RUN-0, RESULTS L59529, hybrid MODEL-LEGAL-RANKING controller 85 / 96 v ENGINE-HCE 92 / 96 v RANDOM-LEGAL 30 / 96, discordants 7 / 0, single checkpoint, shallow loop (one decision on 81 / 92 and 78 / 85 solves), and dropping two encoder-failure roots leaves 5 / 0 below threshold, so the MODEL-WORSE reading is unresolved by the house fence while model >> random is not in doubt; (representation) MATH-CYBER-1-SVP-ADJUDICATION-0, RESULTS L48429, PROGRAM (a <= 8-token canonical instruction) ranks the true sibling first 65 / 72 v STATE 45 / 72, 0 v 20 discordant, one paired birth; negative (1) MATH-CYBER-1-SVP-FREE-ACTION-1, RESULTS L58370, syntax perfect 96 / 96 and content 0 / 96 on every checkpoint, a well-formed command executed at the wrong site. Existing banks this one extends rather than duplicates: PROSTHETIC DIET (RIFF L1400), DELEGATION as the escape from finite density (L1438), THE DUO ARCHITECTURE (L96, NO-TEST by design), expert delegation / sub-agents within a model (L2266), verification inside the forward pass (L2346), the temporal pincer (L2701). No bank exists for reward-minus-compute-cost; that axis is new here.
  Honest breaks: the reverse / backward leg of the pincer is a booked negative family (RESULTS L6599 reverse rows toxic, L6819 11% backward validity, L7010 MEET-V1 fails, L23197 P-NULL hardens), so nothing here should lean on reverse-LLMUE; the flip census (L3062) counts 100,884 flips solidly but its +2 proxy-gate delta is under the fence and must not be quoted as capability; the render atlas (L65859 to L66247) is surface sensitivity of a frozen checkpoint, not a control plane, and is excluded; DA's own numbers are zero-shot on models never trained for the protocol, one context-chunking scheme, thinking disabled; in the house, "the model declared" has so far meant the model ranked among legal actions, not that it emitted a resource request that a runtime then honored.
  Testable residue (a zero-training CAUSAL FACTORIAL on an existing instrument, not a launch): fix one closed-system population with an exact oracle and a runtime that can honor a declaration (the legal-trie decoder of L58795 with a declared SUBSET of the legal set, or the call-span format of L9998 with a declared CALL, or a demand-mask with a model-emitted expert list); arms on the same items and seeds: (i) model declaration + the model-selected resource honored; (ii) the same declaration text + a MATCHED-RANDOM resource of the same size; (iii) the same declaration text + the ANTI-declaration resource; (iv) ORACLE declaration and resource where the oracle can name it; (v) the DA-no-mask analogue, declaration text present, runtime intervention disabled. The exact question this separates: whether the function gain under (i) comes from the runtime honoring WHAT THE MODEL NAMED ((i) > (ii) and (i) > (iii) at matched size, with (v) locating any gain that is only the text) or from the mere presence of a restriction ((i) = (ii)), and how far the model's naming sits below the oracle's ((iv) minus (i)); faithfulness is read from (i) minus (iii) at matched size, never from the declaration text. STOP for the factorial: (i) not above (ii) beyond the gate fence on 3 seeds books "declaration is decoration" for that instrument and the control-plane frame drops to (2) only. Later residue, explicitly held BEHIND the zero-training controls: the TRAINED version, task reward minus lambda times the measured compute or resource cost (attended tokens, engine calls, experts kept, depth used), so the model learns when a global read, a tool, an expert or more depth is worth buying; do not nominate it before the factorial has shown that honoring the declaration is what pays.
  Relation to live programs: independent of ATOM-DIET-TRAJECTORY-1 (running) and of the K2 arc (closed at L66521); shares the SVP / closed-loop instrument family and the MoE identity instruments; nothing here is armed, preregistered or launched.
  Attribution: Artin (frame, generalization, factorial, the reward-minus-cost residue and its ordering), house (paper read, anchor verification, wording).
- **BANKED (2026-09-07): TRAINING-LAW DEGENERACY / WRITER INVARIANCE — the objective constrains BEHAVIOR, the training procedure is a WRITER that selects one implementation among many; which mechanisms are task-forced and which are artifacts of the writer** (Artin: the question, the four-way distinction, the credit-assignment hole hypothesis, the learned-update-necessity desk, the block-swap desk, the interpretation tree, the foreign-writer ranking; house: ledger inventory, literature read, pricing).
  Terminology (replacing "mechanistically faithful", which conflates four things): FUNCTION non-uniqueness, many weight settings realize the same input-output map on the evaluated population; WEIGHT-COORDINATE (gauge) non-uniqueness, permutations and rescalings realize the same function with the same mechanism, already the house gauge law (THEORY "never score weights by distance"); MECHANISM non-uniqueness, different causal implementations (different loci carrying the computation, different intermediate features) realize the same function; WRITER IMPLICIT BIAS, the selection rule by which a training procedure picks one of the degenerate solutions. The question is about the third, and it is answerable only conditional on matched function: given two writers that reach the same verified behavior, is the causal implementation invariant to the writer?
  What the house has ACTUALLY varied (inventory 2026-09-07, every entry verified; all of these compute dL/dW by reverse-mode autodiff and differ in what is done with the gradient or where it is applied): OPTIMIZER, Muon v AdamW (RESULTS L6052 / L6120 / L6137 / L8311, 10 / 34 / 43 v 45 / 57 / 65, a real loss but the same gradient); SCHEDULE, BACKWARD-SCHEDULE-1 (L28261, reversed OneCycle 62 / 120 v 64, one seed, a 2-solve non-difference, not a measured equality) and COMP-LADDER-1 (L28315, C50 60 / C30 49 v 64, one seed); PRECISION / REPRESENTATION, IntAdamW and the deterministic integer birth (L13255), EXACT1 (L23852), ternary from birth (L2281, L2406), SR-BF16 (L7395), rational / Q snap (L9651, L7613); PLASTICITY LOCUS, the control rod (L3195, late-only 71 v full 57), LoRA (llmopt/train/lora.py; DoRA class exists, no booked arm); INITIALIZATION and POST-HOC TRANSFORMS, MERGE-SPACE-1 / -2 (L26770, L26866, independent-init averages 0 / 120 six times, shared init keeps the merge alive), LAM-MERGE (L14091), task vectors (L26770, 1 / 120); DATA / CURRICULUM, the atom, dose, rule-ablation, curric and diet arms (L29290, L29465, L29686, L29929, L28685, L28895, L29113); ONLINE UPDATE, LLMUE and the flip census (L3062, still gradient-signed). CREDIT-ASSIGNMENT variation: NONE. Grep of RESULTS, FINDINGS, THEORY, RIFF, specs, scratch and llmopt for feedback alignment, DFA, synthetic gradient, decoupled neural interface, target propagation, equilibrium propagation, zeroth-order, MeZO, forward-forward, local learning rule, ROME, MEMIT, model editing: zero booked hits. The only non-gradient weight update in the code base is the Hebbian relaxation in llmopt/train/hebbian_moe.py, which runs under no_grad alongside backprop as a regularizer toward co-routed peers, carries no error signal, and was measured function-neutral (RUNG-3 LAMBDA-SWEEP, L12950, "a dial for anatomy, not capability"). Two gradient-free writers are banked and self-declared unrun: ES-LoRA verified hill climbing (RIFF L40, BOARD) and random-perturb-with-engine-acceptance (RIFF L4401, which already names the 19M-dimensional variance break and proposes a d64 structured-perturbation probe). Artin's hypothesis is therefore CONFIRMED from the repository: the credit-assignment law has never been replaced in a matched-function comparison on the same architecture and population. The adjacent inference at RIFF L4404 ("the gradient path's details matter less than believed") rests on two single-seed, sub-fence schedule readings and should not be read as evidence about credit assignment.
  External anchors, read 2026-09-07 (abstracts and, where linked, bodies; none is house-measured): Nøkland 2016, "Direct Feedback Alignment Provides Learning in Deep Neural Networks" (arXiv 1609.01596, NIPS 2016): the output error is projected to each hidden layer through its own fixed random matrix, no backward pass through the forward weights, 1.45% error on permutation-invariant MNIST with dropout. Refinetti, d'Ascoli, Ohana, Goldt 2021, "Align, then memorise: the dynamics of learning with feedback alignment" (arXiv 2011.12428, ICML 2021): DFA first aligns the forward weights so the random feedback becomes useful, then fits the data, and "out of all the low-loss solutions in the landscape, a network trained with DFA naturally converges to the solution which maximises gradient alignment"; the conditioning of the alignment matrices explains its failure on convolutional networks. This is the published statement that a writer breaks the degeneracy by its own bias, which is exactly the object of this bank. Czarnecki, Swirszcz, Jaderberg, Osindero, Vinyals, Kavukcuoglu 2017, "Understanding Synthetic Gradients and Decoupled Neural Interfaces" (arXiv 1703.00522): synthetic gradients do not reduce representational capacity, yet lead to "drastically different layer-wise representations" from backprop on practical problems. Wang et al., "Streamlined optical training of large-scale modern deep learning architectures with direct feedback alignment" (PNAS 2026, arXiv 2409.12965): a hybrid in which 330M of a 1.07B-parameter Transformer's parameters receive the optical DFA signal directly while the rest use local backprop inside each block and the last layer uses BP, with final losses comparable to BP on the Cornell Movie-Dialogs corpus; the feedback matrices are fixed and uncalibrated; no representation or alignment comparison against BP is reported. Malladi et al. 2023, MeZO (arXiv 2305.17333, NeurIPS 2023): zeroth-order SPSA on the parameters with inference-only memory, competitive with fine-tuning up to 66B on pre-trained models with task prompts, which is the regime that makes the classical zeroth-order slowness bearable. Lee, Zhang, Fischer, Bengio 2015, Difference Target Propagation (arXiv 1412.7525): layer-wise targets through per-layer approximate inverses with a difference correction; needs a decoder per layer. Scellier, Bengio 2017, Equilibrium Propagation (arXiv 1602.05179): energy-based model, free and nudged phases, local contrastive update; needs an energy function and convergence to a fixed point. Meng, Bau, Andonian, Belinkov 2022, ROME (arXiv 2202.05262, NeurIPS 2022): causal tracing localizes factual associations to mid-layer MLPs and a closed-form rank-one key-value insertion edits them, a writer that compiles an association into weights without SGD.
  Honest breaks: matched function at the 120 gate is a NON-DIFFERENCE at sigma about 5, never a measured equality, and the house resolution unit is 7 solves; "same function on the gate" is not "same function" (the gate is 120 items in five levels); mechanism readouts must be causal (revert, swap, ablate and re-gate), never weight distance, per the gauge law; a foreign writer that cannot enter the function band licenses no mechanism comparison at all; the mps substrate is run-level nondeterministic, so writer-to-writer differences must clear the same-seed rerun noise (stock s3 v softspeed control s3, RESULTS L66740); ROME-class editing is a different category (targeted association insertion), not a trainer, and belongs in a separate cell.
  Cheapest testable residue, ZERO TRAINING (the writer-invariance desk; priced in OBSERVATION WRITER-INVARIANCE-DESK-0, RESULTS same commit): learned-update necessity on existing same-init specimens. For a specimen sharing W0 with its twin, delta = W_trained minus W0; revert ONLY one module group's delta to W0 and exact-gate the reverted model in-process; necessity_l = gate(full) minus gate(reverted l); compare the necessity PROFILE across writers that reached the same function band. Second desk where the same init allows it: swap one learned block delta from writer A into writer B and gate the hybrid, asking whether two functionally similar solutions are mechanically interchangeable or co-adapted. Interpretation tree: same function AND same causal necessity structure across radically different writers = evidence the mechanism is task- or architecture-constrained rather than optimizer-contingent; same function AND different necessity structure = direct evidence of WRITER-CONTINGENT MECHANISM, behavior does not identify one learned mechanism; writer cannot reach the matched band = no mechanism comparison licensed. STOP for the desk: if the necessity profiles of the two most different same-init, function-matched writers differ by less than the same-seed rerun pair (stock s3 v softspeed control s3) differs from itself, the desk cannot resolve writer contingency at 19M and the question waits for a genuinely foreign writer.
  Later residue, BANK ONLY, not preregistered: a FOREIGN CREDIT WRITER on the house closed-system model, ranked by distance from backprop against instrument cost, audited 2026-09-07: (1) Direct Feedback Alignment, a radically different credit path with the ordinary forward network kept, cheapest to implement (fixed random feedback per block, error projected from the output), with Refinetti's alignment-then-memorise dynamics as the published expectation and the PNAS hybrid as the existence proof at Transformer scale; (2) synthetic gradients / decoupled neural interfaces, local predicted credit, interesting precisely because Czarnecki et al. report different layer-wise representations, at the cost of a gradient-predictor per block; (3) a MeZO-class zeroth-order writer, no backward pass at all, likely far more expensive at 19M from a random init (MeZO's regime is a pre-trained model with a task prompt; RIFF L4401 already names the variance break); (4) target or equilibrium propagation, conceptually strongest but requiring per-layer inverses or an energy-based architecture, so no longer the same model; (5) a direct algebraic / model-editing writer (ROME-class), a distinct category rather than a trainer. Laws for any matched-writer rung, to be carried verbatim into its pre-reg: same architecture; same train and eval population with the gate-band excision; byte-identical initialization (manual_seed then build_model on CPU); the exact function oracle; compare at MATCHED FUNCTION (a predeclared gate band), never at equal step count; never score success by weight distance; a causal mechanism readout (the necessity and swap desks above) is required; strict STOP if the foreign writer cannot enter the predeclared band. The core question is not whether another optimizer scores well; it is whether, conditional on two weight-writing laws producing the same verified behavior, the causal implementation is invariant to the writer. MEASURED 2026-09-09 (VERDICT WRITER-DFA-1, RESULTS L68802): candidate (1), macro DFA under the sealed feedback law, is NOT ACCESSIBLE at 19M / d 384 / 15,420 steps: all four registered (s, lr) cells at seed 21 gate 0 / 120 with loss at the unigram floor, so the strict STOP fired at qualification and the matched-function comparison was never run; the core question stays OPEN, unanswered rather than refuted. Still open on this list: candidates (2) to (5), a hybrid DFA (true gradient in the last block, Launay's hybrid), and a DFA regime with its own accessibility pre-reg (width, horizon, normalization); each needs a fresh pre-reg and Artin GO. POSTMORTEM 2026-09-09 (OBSERVATION WHY-DFA-FAILED-DESCRIPTIVE-0, L69008): no alignment at any scale (max |A| 0.034) and residual-stream rank collapse to about 1 by step 463 in all four cells; the hybrid DFA ladder is now designed as PRE-REG CREDIT-ANCHOR-FRONTIER-1 (L69122, design only, GO required); the DFA regime repair is closed unless a new failure class appears; synthetic gradients / DNI banked below as the next foreign family. FRONTIER RESOLVED 2026-09-09 (VERDICT CREDIT-ANCHOR-FRONTIER-1, L69409): FRONTIER-CLOSED, all six hybrid cells (k = 1, 2, 4) gate 0 / 120 while the frozen-lower zero-credit controls gate 28 / 55 / 62; the DFA-trained lower stack is actively harmful at every k under the sealed interpretation; DFA-family births STOP; the hybrid-DFA candidate and the DFA regime repair are both closed; synthetic gradients / DNI is the next foreign family (Artin decision). AUTOPSY 2026-09-10 (OBSERVATION DFA-LOWER-HARM-DESK-0, L69604): same-init transplants show the DFA-trained lower package is sufficient to destroy a working backprop top (0 / 120 at every k) and W_0 restoration below a failed hybrid top rescues nothing; the rank-collapse reading is supported under the registered law (hybrid stream rank 1.0 to 1.6 everywhere v 20 to 23 in the frozen controls); DFA family closed on evidence, not only on accessibility. CANDIDATE (2) MEASURED 2026-09-11 (VERDICT SYNTHETIC-GRADIENT-WRITER-1 qualification, RESULTS L70832): synthetic gradients / DNI in the top-four-over-frozen-backbone arena is ACCESSIBILITY-ONLY at seed 27 (control 59 / 120; SG cells 1 / 0 / 0 / 0); the strict STOP fired at qualification again and the matched-function comparison was not run; the core question stays OPEN. Still open on this list: candidates (3) to (5) and the SG second design points.
  Relation to programs: complements ATOM-DIET-TRAJECTORY-1 (a DATA writer with backprop fixed; if its STRUCTURAL bar fires, the writer desk asks whether the same anatomy appears under a different credit law); complements the K2 arc (an observational lifecycle under one unknown writer); reuses MERGE-SPACE's same-init law (THEORY row "merge-space is addressed at initialization") and the gauge law. THEORY: no new row; the gauge law row could gain a connection sentence ("writer implicit bias is the selection rule over the gauge-degenerate set; Refinetti et al. 2021") only after the desk or a foreign writer produces a house measurement, per the measured-anchor doctrine. Nothing here is armed, preregistered or launched.
  Attribution: Artin (question, distinctions, hypothesis, desks, interpretation tree, ranking), house (inventory, literature, pricing).
  MEASURED (2026-09-07, VERDICT ATOM-DIET-TRAJECTORY-1, RESULTS L66991): residue (1) ran to completion (six births, 24 post-hoc gates, census, verifier) but books NOT-ADJUDICABLE under its own sealed abort clause: a documentation-only commit (the two banks above and OBSERVATION WRITER-INVARIANCE-DESK-0) landed on the live checkout while birth 2 trained, so births 2 to 5 carry a code_commit different from the launch commit and book NOT-RUN by the sealed stopping law, with the instrument byte-identical at both commits and the verifier's 25 discrepancies all of the commit-identity class. The seed-5 pair is the one adjudicable cell (+6, L4 10 v 8). Descriptively the six-birth data show the REFUTED-IF pattern the registered priors predicted (lift +6 / +8 / +6, S1 ratio 0.447 never above 1, cosines 0.92 / 0.26 / 0.08, centroid signs mixed), recorded as the comparison target for a re-run of the four affected births at one fixed commit. No bar is adjudicated; residue (3) (the STOP of the K2-derived signature-as-lever program) is NOT triggered and residue (2) (the DATA x locus interaction) stays banked and unarmed. Lesson: no commit to a live registered checkout; the driver's guard must compare HEAD to the launch commit at every receipt write.
  MEASURED (2026-09-08, VERDICT ATOM-DIET-TRAJECTORY-1-REPAIR, RESULTS L67274): residue (1) CLOSED on the provenance-repaired evidence set (one code_commit, verifier VERIFIED 0 discrepancies). FUNCTION fires (+6 / +3 / +10, mean +6.33; L4 10 / 11 / 13 v 8 / 7 / 5), STRUCTURAL no-fires (S1 ratio 0.425, never above 1 at any snapshot; S1b direction reproduces at cosines 0.90 / 0.30 / 0.10; S2, S3 no-fire). The REFUTED-IF branch landed as the registered priors predicted, so residue (3) applies: the K2-derived macro-trajectory-signature-as-data-lever program STOPS at this house scale. Residue (2), the DATA x locus interaction test, stays banked and unarmed: P8 / C8 identified no structural locus, so a locus intervention needs a finer instrument first. The K2 bank's renormalization analogy (L9112) gains nothing; the observational K2 transport (L66521) stands as a fact about released checkpoints, not as a lever.
- **BANKED (2026-09-08): COUPLED LEARNING DYNAMICS / ENDOGENOUS DATA — in a closed loop the data is a function of the current weights, so the state is X_t = (W_t, D_t, optimizer and environment state), and the CROSS-FOSTERING 2 x 2 that separates a writer effect from the data the writer caused itself to see** (Artin: the question, the causal notation, the state-space framing, the cross-fostering design and its purpose; house: the loop census, the replay-vehicle audit, wording).
  The mapping, in causal notation. Ordinary offline training: B_t ~ a fixed dataset and order law, g_t = grad_W L(W_t; B_t), W_{t+1} = U(W_t, g_t); the batch CONTENTS are exogenous to W_t even though their gradient effect depends on W_t (house control class: stock_epoch_stream(n_enc, ep) shuffles with random.Random(ep) and takes no model input; every birth19m arm and ATOM-DIET-TRAJECTORY-1 are in this class, and assert_noop pins the stream tuple for tuple). Endogenous / closed-loop training: D_{t+1} = G(W_t, environment, randomness), W_{t+1} = U(W_t, D_{t+1}); the state is X_t = (W_t, D_t, m_t, v_t, RNG, cursor), not W_t alone. NOTATION UPDATE 2026-09-14 (Artin, prospective, framing not a measured law; PRE-REG OPTIMIZER-MEMORY-ABLATION-1, RESULTS L74310): the writer is written STATEFUL from here on, (W_{t+1}, O_{t+1}) = U(W_t, O_t, D_t) with O_t = (exp_avg m_t, exp_avg_sq v_t, Adam step counter, scheduler phase); the earlier W_{t+1} = U(W_t, D_t) form above is retained as written and read as the O-suppressed shorthand. "Coupled" and "endogenous" are the terms; "entanglement" is not used. Objects worth asking about on X_t rather than W_t: fixed points (W*, D*) with D* = G(W*) and W* = U-stationary on D* (the self-consistent data distribution, the honest analogue of a self-consistent field); cycles (a model that alternately over- and under-produces a family); path dependence and hysteresis (the same W reached through different D histories carries different optimizer state and different D_t, so the next step differs); attractors and their basins in X-space; and WRITER-DEPENDENT DATA DISTRIBUTIONS, the point that motivates the factorial below: two writers that generate their own experience are compared on different data even at equal step counts.
  House loop census (verified 2026-09-08 against the code): GENUINELY endogenous, D_{t+1} = G(W_t): the GRPO mining loop (scripts/step_grpo_micro.py, rollouts sampled from the live model and the chain root advanced by the model's own accepted step; seeds arithmetic in SEED_BASE, cycle and ply, so D is a deterministic function of W_t); LLMUE / metabolic v5 (scratch/metabolic_v5.py, rollouts from the live model, the row weight 1 minus the model's own verification rate, negatives the model's own rejects, admission gated by that weight; the episode count is wall-clock bound, so not replayable at fixed seed even in principle); practice mining and the duo miner (scratch/practice_mine.py, scratch/duo_mine.py, the latter the only house data law with TWO models inside G); CURRICULUM-1 arm B (scratch/birth19m_curric.py, level admission triggered by the model's own loss plateau; string-seeded shuffles, so the stream is a deterministic function of the admit_log, which the receipt persists); expert-iteration rounds (docs/LOOP-LOG.md, each round mined by the previous round's model; the "own r23 diet" control retrained on a byte-identical corpus is the house's strongest frozen-endogenous-stream statement, a self-replay, not a cross-replay). PARTIALLY endogenous: the exchange (scratch/exchange_test.py, worklist from a prior model's failures, but the trained rows come from axiom's engine, read once and shuffled by a string seed: exogenous at train time; the 23 chain rows are tracked at data/stuck_chains_p1.jsonl); the metallicity diets (origin endogenous, frozen with a per-file sha manifest, the only sha-anchored frozen stream of this class; its variable is diet degradation, not writer identity). Cross-replay of an A-generated stream into a B writer or a B init: NEVER RUN; the only "open v closed loop" phrase in the ledger (RESULTS L18213) is inference-time expert residency, not a training data law; RESULTS L28256 already registers the gap ("needs a data-replay arm with its own pre-reg").
  Measured anchors: the flip census and LLMUE gain (RESULTS L3062, L3174; flip count solid, the +2 proxy delta under the fence); the exchange (BOARD "2 -> 6 / 12 in 10 min on 23 engine rows", counts re-verified against the tracked file); LOOP-LOG consolidation rows (61 -> 64 -> 65, the final step a 1-solve single-seed delta, unfenced in LOOP-LOG); CURRICULUM-1 arm B 63 / 120 at -1, self-fenced as an almost-stock stream (L28729). None of these separates a writer effect from an endogenous-data effect.
  Honest breaks: every endogenous stream in the house except data/stuck_chains_p1.jsonl and the metallicity manifest is untracked and unsha'd (micromodel_grpo_mined.jsonl 98,528 rows, practice_rows_v5.jsonl 10,344 rows, stuck_states_v5.jsonl, duo_mined_shard1.jsonl); the GRPO loop persists only its SFT by-product, not the reward-bearing groups it trained on, so a replay from that file would answer a different question; the append-mode corpora carry no cycle, seed or source-checkpoint field, so no episode can be sliced out; metabolic v5 is wall-clock bound; mps training is run-level nondeterministic, so an exact replay of the WRITER half needs CPU or cuda with determinism flags (the epsilon-twin fence of the momentum-space bank, RIFF 2026-08-17).
  Testable residue, the CROSS-FOSTERING 2 x 2, banked not launched: two writers (or two inits) A and B each run their closed loop and their actual experience streams are FROZEN with per-episode provenance (cycle, seed, source-checkpoint digest, row sha); then four arms on the same init law and the same gate: A writer + A-generated data, A writer + B-generated data, B writer + A-generated data, B writer + B-generated data, each replayed as an EXOGENOUS fixed stream. Reads: the WRITER effect under identical experience (row contrasts), the ENDOGENOUS-DATA effect (column contrasts), and the WRITER x SELF-GENERATED-DATA interaction (the diagonal minus the off-diagonal); a closed-loop advantage is attributed to a writer only if it survives the swap of data. Cheapest existing vehicle where a model-dependent stream is already exactly recoverable: CURRICULUM-1 arm B replayed from its admit_log (string-seeded shuffles, fixed diet, pinned init; lacks only an ADMIT_SCHEDULE replay switch, an end-of-run-only receipt, and a diet sha on the log), one 19M birth per arm; but that stream is about 83% stock, so it is an instrument demonstration, not an endogenous-signal test. The scientifically loaded vehicle is the GRPO loop, which first needs the logging law: persist the trained groups per cycle with seed and source-checkpoint digest, a step-count bound instead of wall clock, and the identity guard as one shared symbol. STOP for the factorial: if the row contrast (writer at fixed data) and the column contrast (data at fixed writer) are both inside the gate fence at n = 3 paired seeds, the loop's advantage is not decomposable at this scale and the question closes as unresolvable here. MEASURED 2026-09-13 (OBSERVATION VERIFIED-ENDOGENOUS-DATA-CROSSFOSTER-1-DONOR, RESULTS L72632): the donor-library stage of the cross-fostering design was built and run on the census's writer pair (forward v backward OneCycle, shared seed-2 W_0) with one identical verified one-step generator: DONOR-HISTORIES-DEGENERATE, 93.1 % of the 6,000 matched rows identical, union overlap 89 to 92 %, no cross-model CE self-preference. The 2 x 2 is NOT BANKED as designed: under this generator the endogenous history is a function of the roots and the oracle, not of this (weakest) writer contrast, so the column contrast would be empty by construction. Residue: a stronger writer contrast, a multi-ply chain generator, or an all-PASS unmatched-dose library are the redesigns that could separate histories; each is a new pre-reg and GO. The libraries are frozen at data/crossfoster1/. MEASURED 2026-09-14 (VERDICT VERIFIED-ENDOGENOUS-DATA-CROSSFOSTER-1-CHAIN-DESK, RESULTS L72990): the multi-ply residue was run as a zero-training chain desk on the same donor pair (paired verified chains, common seeds, each donor its own state): CHAIN-DEGENERATE, 772 of 6,000 projected rows donor-specific (12.9 %, bar 3,000); iteration doubles the one-step contrast but chains end within 2 to 4 verified steps on the house root generator, so the 2 x 2 stays unbanked and the program is PARKED. Residue now: only a root generator whose paired chains reach 6 or more plies could let the compounding grow; a stronger writer contrast and unmatched-dose libraries remain unmeasured and unlicensed.
  Relation to programs: independent of the closed ATOM-DIET-TRAJECTORY-1 program; a precondition for any WRITER-INVARIANCE test that uses a closed-loop writer (the writer desk and the DFA rung use exogenous streams and are unaffected); complements the momentum-space phase-portrait bank (2026-08-17), which treats S_{t+1} = F(S_t) with data cursor as state but holds the data law exogenous. Nothing armed.
  Attribution: Artin (question, notation, design), house (census, audit, wording).
- **BANKED (2026-09-08): A WEIGHT-SPACE HAMILTONIAN WRITER, HONESTLY PRICED — an artificial Hamiltonian over network weights whose potential is the verified loss, its imaginary-time evolution as a population / distributional writer in a k-dimensional reduced weight space, as a WRITER-INVARIANCE vehicle; plus the SCF analogy audited** (Artin: the question "could learning be a self-consistent or ground-state problem rather than a gradient writer", the formal candidate, the dimensionality warning, the reduced-space idea, the SCF question; house: the ledger audit, the reframing verdict, the pricing).
  The formal candidate, stated so that it cannot be misread: V(W) = L(W; D), the verified task loss; H_W = -(epsilon^2 / 2m) Laplacian_W + V(W); H_W psi = E psi; imaginary-time evolution d psi / d tau = -(H_W - E_ref) psi. This is an ARTIFICIAL Hamiltonian over neural-network weight configurations; psi is an amplitude over possible weights, not a physical wavefunction; low-energy mass concentrates near low-loss regions; imaginary-time evolution is a diffusion-plus-branching (killing) process on W, the diffusion Monte Carlo picture. [WITHDRAWN 2026-09-08, Artin: the original sentence here related psi_0 of this loss-potential Hamiltonian to the Langevin Boltzmann density by psi_0 = sqrt(p); that identification is not established for H_W = -a Laplacian + L and is replaced by the PRECISION paragraph at the end of this bank.] Nothing here implies that a network obeys quantum mechanics; the correct precise statements are (i) SGLD samples the Boltzmann density in the long-time limit (Welling and Teh 2011), (ii) that density is the ground state of a Schrodinger-form operator after the transform (stochastic quantization, Parisi and Wu 1981), (iii) plain gradient descent is the zero-temperature drift of the same Langevin equation and is NOT the imaginary-time evolution of psi. What the house has done (audit 2026-09-08): THEORY row "Training relaxes to a Boltzmann distribution over the oracle-sculpted loss" carries three citations and the status "direct application" but NO measured anchor; the Schrodinger dictionary bank (RIFF L75, 2026-07-19) states "imaginary-time Schrodinger evolution IS gradient descent" as a theorem with a one-parenthesis sketch and no derivation, and borrows "the law we measured four ways" from the GAUGE law (permutation degeneracy: the weight-reader ablation, the nf4 distance null, the committee lottery redraw, the neuron biography), none of which measures an energy, a temperature, a stationary weight distribution or a ground state; llmopt.quantum is a TFIM model-Hamiltonian variational engine on a 2^n statevector (RESULTS L1318, L1340; n <= 12; HVA 0.69% at criticality with six parameters) and was NEVER applied to network weights as the configuration space; the only temperature knob turned on a real objective is the search-side Metropolis population (RESULTS L419, monotone null, "every degree of temperature hurts"), and the momentum-space bank (RIFF 2026-08-17) explicitly calls training "dissipative, NOT Hamiltonian". So the neural-weight Hamiltonian / imaginary-time WRITER has not been run, and the existing THEORY row is a mathematical reframing, not a house measurement (flagged in OBSERVATION COUPLED-LEARNING-AND-HAMILTONIAN-WRITER-ASSESSMENT-0, same commit; a correction paragraph is appended to the RIFF L75 bank).
  Dimensionality, priced honestly: a psi over the 18,911,616 coordinates of the 19M model is not a PDE or an eigenproblem anyone solves; a grid with 3 points per axis has 3^(1.9e7) cells. The smallest scientifically meaningful realization is a REDUCED SPACE W(z) = W_0 + sum_{i=1..k} z_i V_i with k of 2 to 16, V(z) = L(W(z); D), H_z = -(epsilon^2 / 2m) Laplacian_z + V(z). Basis candidates, only where justified: same-init learned block deltas (the eight block deltas of phase_s2 and of backsched_s2 give k = 16 with the two trained models at z = e_A and z = e_B, so the ground state is asked whether it lands on a parent, between them, or elsewhere in their span); trajectory PCA directions of the ATOM-DIET-TRAJECTORY-1 snapshots (17 per birth, already on disk); low-rank update bases; registered task-vector directions; random orthogonal directions as the control basis (the ground state in a random plane through W_0 should be near W_0 if the basis carries no signal). Solvers: exact grid eigensolve only at k <= 2 (a 41 x 41 grid is 1,681 loss evaluations; on a 2,048-row fixed batch set at 19M that is about 4 s each on mps, 1.9 h, or 14 min on a 256-row subset; the eigenvector then reads off in milliseconds); diffusion Monte Carlo / branching random walk at k of 4 to 16 (256 walkers x 200 steps = 51,200 loss evaluations, about 1 h on a 32-row batch); a Langevin / SGLD baseline in the same z-space at the same epsilon; a variational family only if needed. The comparison object is never psi itself: it is the WEIGHT the writer delivers (the mode or a sample of psi_0), gated by the exact oracle (67 s per gate) and then read causally with the learned-update necessity desk (19 gates, 21 min) and the block-swap desk, exactly as for any other writer. The scientific question is not "can quantum mechanics train a network"; it is "can a population / distributional ground-state writer reach matched verified function by a materially different update law, and if so does its causal learned implementation match an ordinary gradient-written model", which makes this a WRITER-INVARIANCE vehicle and nothing more. Never compare by raw weight distance.
  The SCF analogy, audited: Hartree-Fock iterates P_t -> F[P_t] -> generalized eigenproblem -> C_{t+1} -> P_{t+1}, an operator rebuilt from the current state and solved to convergence. A neural analogue (W_t, D_t) -> O_t -> solve -> W_{t+1} -> rebuild O_t is only a new writer if the solve is not a gradient step in disguise. If O_t is the Hessian, Fisher, Gauss-Newton or a data covariance and the solve is one linear system, the loop is Newton / natural gradient / K-FAC / preconditioned descent: known methods, not "SCF training", and the house has none of them booked (Muon's orthogonalized update is the only preconditioner variation, RESULTS L6052 to L8311, a loss). Two candidates that are genuinely distinct and testable: (a) the DATA-SIDE self-consistent field, the fixed point (W*, D*) of the coupled map D = G(W), W = argmin L(W; D), where the "field" is the model's own generated data distribution; its existence, uniqueness and basin are the objects of the endogenous-data bank above, and the SCF iteration is exactly the closed loop the house already runs without ever asking whether it converges; (b) a layer-wise closed-form solve writer, each layer's weight taken from a linear or eigen solve of its own input covariance against a target that the current model supplies, iterated to self-consistency, which is a full solve per sweep rather than a step and has published kin in forward-only and closed-form stacking methods; it ranks below DFA on the foreign-writer list because it changes the objective per layer, not only the credit path. Neither is armed.
  Honest breaks: matched function at the 120 gate is a non-difference at sigma about 5; the reduced space with learned-delta bases is not a foreign writer in the strong sense (the parents span the basis), so a ground state that lands on a parent is uninformative and the random-basis control is mandatory; DMC weights are population statistics, not one network, and the delivered network must be one sample or the mode, declared before the run; epsilon and m are free knobs that must be registered (they set the temperature); the k = 2 exact solve is a demonstration, not a writer at scale; the analogy to quantum chemistry stops at the variational principle and the diffusion picture, and any "psi of the network" language stays out of ledger claims.
  Testable residue, ranked below WRITER-INVARIANCE-DESK-0 (the zero-training schedule-writer necessity desk) and below the DFA foreign-writer rung: (1) k = 2 exact eigensolve in the plane of the phase_s2 and backsched_s2 delta vectors (same W_0), 41 x 41 grid on a registered 256-row batch, one gate of the ground-state weight, then the 19-gate necessity desk on it; STOP if the ground-state weight gates below the parents' band minus one resolution unit (below 57 / 120), since no mechanism comparison is then licensed; (2) the same at k = 16 over the sixteen block deltas by DMC with the SGLD baseline; (3) random-basis control at the same k. Nothing armed.
  Relation to programs: a candidate foreign writer for the WRITER INVARIANCE bank (2026-09-07), not a replacement for its desk or its DFA rung; independent of the closed ATOM-DIET-TRAJECTORY-1 program.
  Attribution: Artin (question, formal candidate, reduced-space idea, SCF question, dimensionality warning), house (audit, pricing, wording).
  PRECISION (2026-09-08, Artin-directed; documentation only, nothing armed). Two constructions that this bank must never conflate. (A) The ARTIFICIAL LOSS-POTENTIAL HAMILTONIAN H_loss = -a Laplacian_W + L(W): a deliberately constructed quantum-style optimization problem; its ground-state density |psi_0|^2 is NOT in general the Boltzmann density exp(-beta L), and no sentence in this ledger may write "H = -a Delta + L, therefore psi_0^2 ∝ exp(-beta L)" unless a derivation under a stated special case establishes it. (B) The LANGEVIN / FOKKER-PLANCK -> SCHRODINGER TRANSFORM: for overdamped Langevin dynamics in the loss L at temperature T the stationary density is p_inf ∝ exp(-L / T); after the standard similarity transform (psi = p / sqrt(p_inf), or the equivalent convention) the Fokker-Planck generator becomes a Schrodinger-form operator whose EFFECTIVE potential contains gradient-norm (|grad L|^2) and Laplacian-of-loss terms, and whose ground state is proportional to sqrt(p_inf). The two operators differ by exactly that effective potential; (B) is a theorem about Langevin dynamics, (A) is a design choice. Gradient descent is the deterministic, zero-noise drift cousin of (B); imaginary-time evolution under (A) and Langevin / Fokker-Planck under (B) are different dynamics; the withdrawal of "imaginary-time Schrodinger IS gradient descent" (RIFF L76) stands. Definition of "ground-state weight", frozen BEFORE any rung: an eigensolver returns psi_0(z), a function on the reduced space, not one checkpoint, so a pre-reg must register an EXTRACTION LAW, at minimum both of z_mode = argmax |psi_0(z)|^2 and z_mean = E_{|psi_0|^2}[z], emitted and gated as two separately registered descriptive outputs (they differ for asymmetric or multimodal densities; neither is chosen after viewing the landscape). Registered as part of the writer, not as incidental numerics: the boundary conditions on the grid (Dirichlet at the box edge or otherwise), the coordinate normalization of each V_i (unit Frobenius norm or the raw delta), the grid bounds in z, the kinetic coefficient a (or epsilon^2 / 2m), and whether V(z) is a fixed registered batch loss or the exact full registered objective. The k = 2 desk residue above inherits all of this: both extraction laws, both gated.
  Attribution of the precision note: Artin (the two-construction separation, the extraction-law requirement, the list of registered knobs), house (wording).
- **BANKED (2026-09-08): THE MECHANISM-ATTRIBUTION PROGRAM — M = M(TASK, WRITER, ENDOGENOUS-DATA GENERATOR, W_0, substrate noise), built sequentially from a zero-training trajectory-derivative census on existing same-init checkpoints, through a foreign credit writer, a task intervention and cross-fostered endogenous data; plus the exact derivative distinctions** (Artin with a GPT seat: the decomposition, the staged build, the trajectory-derivative census, the noise-null law, the derivative and initial-condition question, the DFA design constraints, the task-intervention and cross-fostering stages, the caution against a premature variance decomposition; house: the artifact audit, the reuse of PHASE-PORTRAIT-1 and WRITER-INVARIANCE-DESK-0, the prereg).
  The frame: mechanism M is an outcome; the causal factors are T (task, the target computation), U (the weight-writing / credit-assignment law) and G (the endogenous-data generator, the experience-selection law), with W_0 held byte-identical wherever possible and substrate noise measured by same-writer reruns. The eventual organizing form Mechanism = Task + Writer + EndogenousData + Task x Writer + Writer x EndogenousData + residual is the CAUSAL ORGANIZATION of the program, not a promised variance decomposition; no literal decomposition is claimed until the single-factor rungs and their replication support one. Stages: 0 = trajectory-derivative census on the existing same-init forward / backward OneCycle milestones with the four same-writer rerun pairs as the checkpoint-level noise null (PRE-REG WRITER-TRAJECTORY-CENSUS-0, RESULTS L67576, zero training); 0B = the finished-model learned-update DEPENDENCE profile (D_l = gate(full) minus gate(group l reverted); "dependence", not "necessity") with bidirectional block swaps read as cross-writer component COMPATIBILITY, never as an algorithm claim (co-adaptation, basis and scaling stay live); 1 = the first FOREIGN writer, Direct Feedback Alignment unless a cheaper valid credit replacement is found, under identical architecture, tokenizer, byte-identical W_0, identical batch ids and order, same objective and exposure, same gate, output-head treatment frozen, feedback matrices from a registered seed, writer hyperparameters qualified without looking at discovery outcomes, and the primary prerequisite that DFA enters a predefined matched-function band (else an accessibility result only); 2 = a task intervention under one fixed writer and byte-identical W_0 using the closest house task pair that keeps architecture, tokenizer and exposure fixed and changes the target computation in a named way (candidates: the rule-family ablation of RULE-ABLATE-1, L29929, and the xterm / basics diet pairs, L30715 / L30498), with the mechanism observables frozen in stage 1; 3 = the endogenous-data logging law first (every trained group, cycle, seed, source-checkpoint state digest, exact row ids and content, acceptance / rejection, writer identity, a deterministic replay path, step-count bounds instead of wall clock), then the cross-fostering 2 x 2 of the COUPLED LEARNING DYNAMICS bank. No 2 x 2 x 2 is built at once.
  Derivative distinctions (Artin's "can we take the derivative of the model weights" and "are trajectories similar because they share initial conditions"): (A) the loss gradient grad_W L(W_t, D_t), what the writer consumes; (B) the observed training velocity [W(t + h) - W(t)] / h, the writer's realized update averaged over h steps (for AdamW not the gradient); (C) the observed acceleration [v(t + h) - v(t)] / h; (D) the local sensitivity of the update map J_t = dU(W_t) / dW_t, never materialized at 19M x 19M, estimable cheaply by a Jacobian-vector product of one update step (torch.func.jvp through forward, backward and the AdamW step with batch and optimizer state fixed; about two training steps per direction) or by the perturb-and-replay finite-time sensitivity lambda_T(v) = (1 / T) log(||dW_T|| / ||dW_0||) along a registered direction, which needs deterministic replay (CPU or cuda with determinism flags, never mps) and is the epsilon-twin residue of the momentum-space phase-portrait bank (2026-08-17), to which the JVP estimator is added as a rider; lambda_T is a sensitivity measure, not a chaos claim. Shared initial conditions make early trajectories similar by construction; only late-time and mid-training statistics against a same-writer null can speak to the writer.
  Measured anchors (verified 2026-09-08): shared W_0 at seed 2 for phase19m and backsched19m (the backward step-1 milestone at lr 1.2e-9 sits 5.5e-6 from the regenerated W_0; the forward one 0.052); 17 milestones per family on the 900 grid with Adam state, plus the step-1 milestone (count corrected by AMENDMENT WRITER-TRAJECTORY-CENSUS-0-SEAL); BACKWARD-SCHEDULE-1 62 v 64 (L28261, a 2-solve non-difference); the four same-writer rerun pairs of ATOM-DIET-TRAJECTORY-1 (first run v repair) at 16 matched snapshots; PHASE-PORTRAIT-1 per-neuron speeds and Adam momentum norms on the forward milestones (portrait-only, no gate). Disclosed peek during feasibility: global cumulative cosine between the forward and backward writers at step 15,300 read 0.211 with relative divergence 1.256; no null value read; the prereg's bars compare against the unseen null.
  Honest breaks: the schedule direction is the weakest writer intervention (both arms backprop + AdamW; CORRECTION 2026-09-14, AMENDMENT WRITER-INTERVENTION-SCHEDULER-LAW L73710: the intervention is stock OneCycle, LR cycled with beta1 cycled 0.95 -> 0.85 -> 0.95, versus backward SequenceLR, LR reversed with beta1 fixed at 0.9, two optimizer fields per step, not one); the null pairs and the writer pair are at different seeds and on different snapshot grids (1,028 v 900), compared at normalized time; trajectory coordinates are licensed only by a shared W_0 (gauge law); dependence profiles are gate differences at sigma about 5 against one null pair; a failed swap is incompatibility, not a different algorithm; the DFA stage owes its own hyperparameter qualification law and a matched-function band before any comparison; stage 2's "task" pairs also change data, so the task factor is confounded with data content unless exposure and row grain are matched by construction.
  Testable residue and STOP / GO: WRITER-TRAJECTORY-CENSUS-0 (L67576) is the smallest rung; GO to a DFA pre-reg only if its verifier is VERIFIED, TRAJECTORY fires and the function band holds; STOP the ladder if TRAJECTORY no-fires (writers unresolvable at 19M) or if the dependence-level null is within 20% of the writer contrast, in which case the next step is three same-writer reruns, not a foreign writer. Nothing armed; Artin GO required after a clean prereg-auditor. UPDATE 2026-09-09: WRITER-TRAJECTORY-CENSUS-0 fired (L67980), NULL-2 held (L68265), the DFA pre-reg was sealed and GO'd, and its qualification ladder STOPPED as DFA-UNSTABLE (VERDICT WRITER-DFA-1, L68802): the foreign writer did not reach a non-zero gate, so stage 1 of this program is closed at accessibility with the mechanism question unanswered; the next foreign writer candidate is Artin's decision.
  Attribution: Artin and the GPT seat (frame, stages, constraints, questions), house (audit, prereg, wording).
- **BANKED (2026-09-08): VERIFIER INVARIANCE / COMPARATOR-LOCKED ORACLE — the same mathematical claim under two independently built verification stacks (the house Lean tier v a Comparator-style challenge / solution separation with kernel-level statement identity, an axiom whitelist and a second kernel), framed like writer invariance: same claim, different verifier, same or different verdict; smallest rung COMPARATOR-LOCKED-ORACLE-0 on the frozen certificate corpus** (Artin: the pointer to openai/NavierStokesAndEuler and its Comparator architecture, the verifier-invariance framing, the candidate rung and bars, the closed discrepancy taxonomy, the scope fence "verifier invariance, not proof correctness beyond the trusted challenge and kernels"; house: the repository audit and the house-tier inventory).
  What the external repository actually shows (audited 2026-09-08 from the files, not the prompt): formalization.yaml v0.4 declares full formalization of the main results with sorry_count 0, four main declarations (NavierStokes.Comparator.navier_stokes_breakdown_R3 and _periodic in NavierStokes/ComparatorSolution.lean; Euler.euler_breakdown_R3 and Euler.exists_compact_smooth_euler_singularity in Euler/Solution.lean), permitted axioms propext, Classical.choice, Quot.sound, comparator_config files, automation "agent: GPT-6 Astra, framework Codex", review "self-assessed", Lean 4.34.0-rc2 with Mathlib; ComparatorChallenges/NavierStokes.lean is a standalone copy of Google DeepMind's Formal Conjectures NavierStokes.lean at commit 8bf45ed7, retaining the upstream definitions and the intentional `sorry` placeholders for alternatives (C) and (D), with the comment "neither the proof root nor the submission imports this reference"; ComparatorSolution.lean imports only the project's own theorem modules and its adapters import ComparatorDefinitions, never the challenge; ComparatorBridge.lean translates the project's divergence, gradient, Laplacian and time-derivative operators, swaps the (time, space) coordinate order, and normalizes any positive viscosity to one; ComparatorChallenges/NavierStokes.json names the challenge and solution modules, the two theorem names, the three permitted axioms and enable_nanoda true. Comparator itself (leanprover/comparator, Lean FRO with the AIMO team) builds the challenge and the solution in landrun sandboxes, runs lean4export on both .olean environments, checks that every declaration referenced by the named theorem statements is identical between the two environments (Init included), checks the solution's axioms against the whitelist, replays the solution through the Lean kernel and optionally through nanoda (an independent Rust kernel); its stated limits: solutions must re-declare all definitions identically, universe and safety levels must match, definition holes need human review, and the trust base includes landrun working correctly. The repository documents no search logs and no lemma library.
  What the house already has (inventory 2026-09-08, file:line verified): PRODUCER (axiom's sym/print_lean.hpp emitter, bound since IV7, RESULTS L29417) -> independent house statement reconstruction (scratch/lean_check.py rederive, its own binder, hypothesis and template printer from (lhs, rhs, atoms, tactic)) -> mechanical statement diff (ac_equal: exact string, else sorted binder set, verbatim hypotheses, sympy parse equality per side with evaluate=True and no simplification; the tactic string is copied from the row, so that component never fails) -> Lean kernel (lake env lean on 50-row chunks of anonymous `example` declarations under `import Mathlib.Tactic`, PASS iff rc 0, failures counted per error line). Booked: LEAN-TIER-1 (L18619, 443 / 443), LEAN-KERNEL-SAMPLE (L20365, 703 / 1000) and its amendment (L20492), LEAN-SAMPLE-V2 (L22407, 989 / 1000), LEAN-FULL-V2 (L22493, 21,614 / 21,914 = 98.63%, closed taxonomy 222 atom-split + 78 field_simp-open, statement diff 15,751 / 21,914 exact or AC), RECEIPT LEAN-300-LABELS (L23777). So "use Lean" and "diff the statement mechanically" are NOT new. ABSENT in the house, each verified by grep: a permitted-axiom audit (#print axioms, sorryAx), a second kernel, a sandboxed challenge / solution separation, theorem names (everything is `example`, identity by a `-- id:` comment), a frozen and hashed import closure (the LEAN-FULL-V2 mathlib revision is unrecorded; the WSL run pinned 9fb10993), kernel-level statement identity (the diff is string / sympy), a kernel timeout.
  The genuinely new residue, COMPARATOR-LOCKED-ORACLE-0 (banked, not preregistered): take a frozen stratified subset of the existing certificate corpus (no model run); generate the trusted Challenge.lean ONLY from the original (lhs, rhs, atoms, hypotheses) rows by the house printer, naming each declaration (theorem cert_<id>) with a `sorry` body, never importing or calling axiom's emitter; generate the untrusted Solution.lean from axiom's emitted certificates under the same theorem names, never importing Challenge; freeze both import closures (lake-manifest and toolchain hashes recorded); run Comparator with a preregistered permitted-axiom list and nanoda enabled if the toolchain is compatible; identity of statements is decided by Comparator's exported-declaration comparison, not by ac_equal. Bars to assess: C1 STATEMENT-IDENTITY (every eligible sampled certificate exposes the exact trusted statement under Comparator), C2 AXIOM-CLEAN (no theorem uses an axiom outside the whitelist), C3 DUAL-KERNEL (Lean and nanoda agree on every C1-clean certificate), C4 OLD-v-NEW (the house tier's PASS / FAIL and the Comparator verdict agree on every eligible specimen). Closed discrepancy taxonomy, preserved and never repaired before booking: challenge / solution statement mismatch; import or declaration mismatch; excess axiom; Lean kernel rejection; external-kernel rejection; sandbox or tool failure. Price: 100-certificate smoke, then a 1,000-certificate qualification (house kernel cost 132 ms per certificate at LEAN-FULL-V2, so the kernel half of 1,000 is minutes; Comparator adds two sandboxed builds and two exports per batch, unpriced until installed: landrun, lean4export and nanoda are not in the house toolchain and Comparator's Lean version must be matched to the leancheck toolchain v4.33.0-rc2), then the full 21,914 only if the 1,000 shows a nonzero discrepancy class or C4 disagreement worth the wall.
  Honest breaks: the house corpus is ring-identity certificates, not deep theorems; a Comparator run on them measures the verification STACK, not proof strength; the theorem-name and `sorry`-challenge law changes the house declaration form (example -> theorem), which must itself be shown not to change kernel outcomes (C4 is that check); the house printer's hypothesis derivation is regex-over-text and left-to-right, so a Comparator mismatch class may be a printer-order artefact and the taxonomy must keep it separate; a sandbox that fails is a tool result, not a verdict; nanoda compatibility with the house toolchain is unverified.
  Relation: a cheap parallel verifier-hardening rung that displaces nothing; it is the verifier-side twin of WRITER INVARIANCE (RIFF 2026-09-07). Two defects surfaced by the audit are corrected this commit: the e / m split in LEAN-FULL-V2 (AMENDMENT LEAN-FULL-V2-TAXONOMY-PROSE) and the stale "no house consumer" line in docs/AXIOM-SURFACE.md. Nothing armed.
  Attribution: Artin (pointer, framing, rung, bars, taxonomy), house (audits).
- **BANKED (2026-09-08): VERIFIED ENDOGENOUS LEMMA MEMORY — a truth-filtered closed loop D_{t+1} = D_t ∪ {lemma l generated at t with KernelVerify(l) = PASS}, with the cross-fostered 2 x 2 that asks whether a search law's advantage is its law or the verified knowledge it caused itself to discover** (Artin: the loop, the question, the factorial, the controls, the provenance law, the caution not to attribute a closed-loop lemma mechanism to the OpenAI repository; house: the house-vehicle audit).
  Separation of evidence: openai/NavierStokesAndEuler's final proof is modular and composes verified intermediate constructions, but the repository publishes no search logs and no lemma library, so it demonstrates a VERIFIED MODULAR ARTIFACT, not a measured closed-loop lemma-generation mechanism; nothing here is inferred from it beyond the artifact.
  The experiment, banked independently: a search or writer system may admit into its future search ONLY newly generated intermediate lemmas that passed the formal verifier; question, does a truth-filtered endogenous lemma library improve proof or search performance, and is the improvement carried by the search policy or by the verified data it caused itself to discover. Later factorial: A policy + A-generated verified library, A + B-generated, B + A-generated, B + B-generated, reading the policy main effect under shared verified knowledge, the endogenous-library main effect, and their interaction. Controls: equal library size and token budget; random verified lemmas matched for size; anti-relevant verified lemmas where a principled construction exists; a static exogenous library; the closed-loop own library; exact provenance per admitted lemma (producing checkpoint or policy, theorem id, proof hash, verifier and kernel identity, order and timestamp). This is a cleaner endogenous-data vehicle than unconstrained self-generated text (COUPLED LEARNING DYNAMICS bank, 2026-09-08) because every admitted datum carries a formal truth certificate.
  House vehicle audit (2026-09-08): no lemma library, bank of verified steps, or closed-loop admission of self-generated verified results into future search exists; the only reuse is in-process memoization (llmopt/search/derivation.py _RULE_CACHE bounded at 200,000; llmopt/search/magic.py _VERDICT_CACHE), unlogged and non-persistent. The nearest banked idea is the solved-only mining leak (RIFF L293, verified steps of failed attempts discarded). Candidate vehicles, cheapest first: (1) the axiom calculus search with the Lean-eligible ring-identity subset as the verifier: a lemma = a verified rewrite step or sub-identity certified by the kernel, reusable as a rule in later searches; the search already exposes step chains and engine rows, so the missing pieces are a persistent, provenance-logged lemma store and an admission switch; (2) ActionProgram / MathWorld, whose decoder replays actions but has no verified sub-lemma object yet; (3) any theorem-search environment with reusable intermediate states, none of which the house holds. STOP for the first rung: if the own-library arm does not exceed the size-matched random-verified-library arm on the exact oracle at n = 3 seeds, the library's content is not what pays and the loop reduces to a data-volume effect.
  Relation and ranking: below WRITER-TRAJECTORY-CENSUS-0 and the DFA rung; a possible cleaner replacement for the GRPO cross-fostering vehicle of the COUPLED LEARNING DYNAMICS bank once the logging law exists; above the Hamiltonian writer. Nothing armed.
  Attribution: Artin (loop, question, factorial, controls), house (audit).
- **BANKED (2026-09-08): REPRESENTATION-BRIDGE TRANSPORT (lower priority) — a representation earns stronger semantic status if its decoded action or theorem transports through an independently implemented semantic interface rather than only replaying inside the house decoder; the ComparatorBridge pattern (internal representation -> independently specified target semantics -> proved bridge) as the analogy for ActionProgram** (Artin: the analogy and the criterion; house: the audit of existing interchange work).
  Audit: the house has ONE booked transport through an independent interface, OBSERVATION MATH-CYBER-1-AX-ABI-COUNTERBOOK-0 (RESULTS L46773): the LAW transports (rule semantics agree) while the STATE SPELLING does not (6 / 64 core parents and 183 / 2,894 stress rows admitted by axiom's engine; roundtrip divergence 703 of 725 core rows), explicitly not claimed as a canonical cross-engine ISA; every other ActionProgram result (ACTIONPROG-QUAL-0 L44860 533 / 533, v3 L45392, v4 L45570, FINAL-QUAL L46519 725 / 725 and 2,894, AX-FIXTURE-0 L46685) is replay inside the house decoder. So the criterion is already partly measured and the measurement is the negative half: the bridge exists for laws, not for states. Not redundant, not armed: the open residue is a proved or mechanically checked STATE bridge (a canonical spelling map with a round-trip identity under both engines), which is instrument work on the MathWorld interchange contract, not a rung; it ranks below every item above.
  Attribution: Artin (analogy), house (audit).
  MEASURED (2026-09-09, VERDICT WRITER-TRAJECTORY-CENSUS-0, RESULTS L67980): STAGE 0 and 0B closed. TRAJECTORY fires (C_AB 0.211 v null min 0.777; R_AB 1.256 v null max 0.668; velocity median 0.021 v 0.304; 8 / 8 blocks), DEPENDENCE fires (9-group profile distance 39.4 v same-writer 9.6; the NO-FIRE prior MISSED), COMPATIBILITY fires (cross-writer swap median -11.5 v same-writer worst -7); D-0 band 3. Sealed reading: writer-contingent dependence structure even between two backprop schedules; incompatibility only. The DFA ladder's GO conditions are met and the STOP clause is comfortably not triggered (the clause needs d_null within 20% of d_writer, i.e. >= 31.5; measured 9.64; the earlier 'narrow margin' reading is withdrawn by AMENDMENT -STOP-READING); a second same-writer dependence pair is registered prospectively as PRE-REG WRITER-DEPENDENCE-NULL-2. STAGE 1 (DFA) may now be preregistered; it is not authorized to run.

- **BANKED (2026-09-09): SYNTHETIC GRADIENTS / DECOUPLED NEURAL INTERFACES — the next genuinely different credit writer after the DFA accessibility frontier; a LEARNED feedback law rather than a fixed random one** (Artin: the ranking and the candidate form; house: the literature check and the transport fences).
  The candidate: block l's hidden error is predicted locally, hat_delta_l = G_phi_l(h_l, context), and block l updates by J_{f_l}^T hat_delta_l instead of the immediately backpropagated downstream error; G_phi_l is a separate small module trained against a registered target (the true downstream gradient, or a delayed / bootstrapped target). Because the feedback is learned rather than fixed random, this writer is LESS alien than pure DFA (VERDICT WRITER-DFA-1 L68802: inaccessible at 19M) and more likely to attain matched function, while still replacing the credit path at every block boundary.
  Literature, checked 2026-09-09 from the arXiv abstracts (not the full papers): Jaderberg et al. 2016 (arXiv 1608.05343) introduce synthetic gradients as "modelled synthetic gradient in place of true backpropagated error gradients", predictors trained on the true backpropagated gradients as targets, demonstrated on feed-forward nets trained "asynchronously", RNNs (extends the effective modelling horizon) and hierarchical RNNs; Czarnecki et al. 2017 (arXiv 1703.00522) "prove the convergence of the learning system for linear and deep linear models", state that synthetic gradients do "not affect the representational strength of the learning system", and report that the approximation "leads to drastically different layer-wise representations" from true-gradient training. Neither reports a Transformer; a house implementation does NOT transport unchanged: the predictor's input for a decoder block is a (batch, T, 384) residual stream and its target a per-token hidden error, so G_phi_l must be a per-token map (a linear or one-hidden-layer readout of h_l, optionally conditioned on the label token), its training signal must be taken from a read-only true-gradient pass (the same offline machinery as the WRITER-DFA-1 alignment diagnostic, moved inside the step), and the delay / bootstrapping variants of the papers are RNN-specific and are not part of the first house form.
  Honest breaks: the writer is only foreign to the extent the predictor is not a copy of backprop (a perfect predictor reproduces BP exactly: the frontier lies in predictor capacity and target delay, so the pre-reg must fix both); the predictor is trained on true gradients, so a "true gradient never updates the arm" law of the DFA kind cannot be written and a weaker law (true gradients update ONLY the predictors, never the blocks) replaces it; extra parameters and an extra loss per block change the cost line; at 19M the accessibility question comes first, as with DFA.
  Testable residue: PRE-REG SYNTHETIC-GRADIENT-WRITER-1 after CREDIT-ANCHOR-FRONTIER-1 resolves: same architecture, W_0, stream, cadence and gate; the WRITER-DFA-1 qualification and matched-function laws carried verbatim; the predictor family, its target (true v delayed), and its own learning rate fixed in the pre-reg; the P1-style integrity smoke rewritten for the weaker law; the alignment diagnostic becomes cos(hat_delta_l, delta^BP_l), now expected to rise. Bank only; nothing armed. MEASURED 2026-09-11 (VERDICT SYNTHETIC-GRADIENT-WRITER-1 qualification, RESULTS L70832): on the top-four-over-frozen-backbone arena (AMENDMENT -ARENA L70126, sealed L70425 with the fixed audited constants of L70301, FOLD A same-pre-update teacher order, FOLD B elementwise normalized MSE) the writer is ACCESSIBILITY-ONLY at seed 27: control 59 / 120, LINEAR 3e-4 1, LINEAR 3e-5 0, MLP-256 3e-4 0, MLP-256 3e-5 0, all finite; the predictors never beat the zero predictor beyond 4 % and the alignment never rose above 0.23. The 'expected to rise' residue is refuted on this arena; what stays open: the bootstrapped every-layer target and a delayed target (the named second design points), other predictor families, and the two candidate zero-training leads recorded in the verdict (predictors that do not beat zero; vanishing true hidden errors on the MLP cells). Nothing armed. DESK 2026-09-11 (OBSERVATION SG-FAILURE-DESK-0, RESULTS L71182): the two leads resolve descriptively into one: the per-token (x_l+1, e_t) -> delta_BP map is not representable by a linear or random-feature oracle even on the frozen-BP control once trained (held-out ratio about 1), so the predictors' failure is not a tracking failure (branch A holds nowhere) and the MLP cells' target fall is a self-induced representation collapse of the credited blocks (branch D), not a readout collapse. Open after the desk: whether a cross-position / per-sequence predictor input (the target depends on the downstream attention over other positions, untested) or the named second design points change the representability; every such thing is a new formulation needing its own pre-reg and GO. Nothing armed. CROSS-POSITION DESK 2026-09-11 (VERDICT SG-CROSSPOS-REPRESENTABILITY-0, RESULTS L71557): the cross-position lead is CLOSED at this arena. A fixed-capacity reverse-causal sequence predictor reading [x_l+1, e] at j >= i reaches late median HELDOUT ratio 1.10 on the healthy control (blocks 4..6 worse than zero, block 7 0.10 to 0.12) and its same-architecture LOCAL (j == i) twin is indistinguishable (paired gap median -0.008): the per-block sequence at j >= i does not carry what delta^BP on blocks 4..6 needs, within 500k parameters and 1,200 steps. BAR-1 NO-FIRE => the SG family closes at this arena as sealed; no seed-28 SEQUENCE-SG design banked. Descriptive lead only: block 7 (the block below the head) IS representable from its local input by the trainable family (0.11 in both arms) where the closed-form oracles were not; a one-block SG writer there is a different design needing its own pre-reg. Untested and unlicensed: bidirectional (j < i) input, downstream weights as input, larger predictors. BLOCK-7 BOUNDARY WRITER 2026-09-11 (VERDICT SG-BOUNDARY-BLOCK7-1, RESULTS L71955): the one-block LOCAL design was pre-registered, implemented (true CE through block 7 with detached parameter views so blocks 4..6 / norm / head keep the control's exact gradient; forced-delta endpoint bit-exact), and run at seed 28: SG7-MISS, 37 v control 64 (band 57 to 71) with final CE 0.399 v 0.384 and a predictor that does track block 7's error (MSE ratio 0.58, cos 0.68 at the final). SG CREDIT-WRITER BIRTHS ARE CLOSED COMPLETELY (the sealed consequence): no larger, delayed, bootstrapped, bidirectional or downstream-weight-conditioned repair. Open descriptively, unlicensed: why a block-7-only credit that tracks its target costs 27 solves at near-equal training CE (target scale fall 2.6e5-fold v a fixed s_7; alignment 0.4 to 0.7; gate sensitivity to block 7) is a mechanism question the family no longer licenses a birth to answer. CANDIDATE 3 (zeroth-order / MeZO) DESK 2026-09-12 (VERDICT MEZO-SIGNAL-DESK-0, RESULTS L72285): on the healthy control states the antithetic estimator is faithful and exactly isotropic-SPSA (cosine 5.8e-4 at m = 4, 0.90 of sqrt(m/d); rank-1 structured perturbations no better), and a practical m = 4 step buys 1.6 % of a BP step's line-optimal descent (bar 10 %): PARK. The foreign-writer program (DFA, SG in three forms, zeroth-order) is parked in full; target / equilibrium propagation and ROME are not licensed by this desk and need their own GO and pre-reg.

- **BANKED (2026-09-09): OPTIMIZER-REGIME DFA REPAIR — a literature-anchored repair of the WRITER-DFA-1 failure (optimizer / schedule / normalization / width / horizon), banked, NOT to be launched until the postmortem classifies the failure** (Artin: the hold and its condition; house: the published anchor).
  Anchor: Launay et al. 2020 report macro-DFA Transformers as sensitive to optimizer and schedule choices (Table 5: Adam lr lowered from 1e-4 to 5e-5, beta_2 0.999, with a large perplexity gap remaining), so a house DFA regime that differs in one of these could in principle move the plateau; the four house cells spanned a 16x feedback scale and 3x lr with no effect (L68802).
  Condition for release, registered: the postmortem (OBSERVATION WHY-DFA-FAILED-DESCRIPTIVE-0) must show which failure class this is — no feedback alignment (A flat near zero), bad feedback magnitude (Q far from 1 in a direction a rescaling could fix), representation movement without structured learning (ACT far from W_0, blocks moving), or another identified accessibility defect. Only a class with a preregisterable, implementation-independent lever (e.g. magnitude) releases this bank; "no alignment at any scale" does not, and the CREDIT-ANCHOR-FRONTIER (exact credit in the top k blocks) is the preferred next accessibility rung in every class. RESOLVED 2026-09-09 (OBSERVATION WHY-DFA-FAILED-DESCRIPTIVE-0, L69008): the class is NO ALIGNMENT AT ANY SCALE plus residual-stream rank collapse by step 463 at every s and lr; no magnitude or lr lever exists; this bank stays CLOSED-UNLESS-REOPENED by a new failure class from the frontier; the frontier proceeds. FRONTIER CLOSED 2026-09-09 (L69409): no new failure class; this bank stays closed.
  Honest breaks: any regime change is a new writer for the invariance question (the comparison is at matched function, and the control must share the regime); width / horizon changes move the whole comparison population, not one arm. Testable residue: a one-lever pre-reg named by the postmortem's class; nothing armed.

- **BANKED (2026-09-09): FROZEN-RANDOM-BACKBONE — at seed 23 a frozen random embedding plus four frozen random-initialized bottom blocks, with only the top four blocks + norm + head trained by backprop on the stock recipe, gate 62 / 120, descriptively inside the historical stock 61 to 65 (seeds 5 to 7) but not same-seed replicated** (Artin: the bank, the framing "backbone", and the candidate paired rung; house: the measurement, a registered zero-credit control of CREDIT-ANCHOR-FRONTIER-1, VERDICT RESULTS L69409).
  The measurement: the zero-credit controls of the frontier (lower stack and emb frozen at W_0, top k blocks + norm + head trained by the stock backprop recipe, 15,420 steps, the pinned stream, seed 23) gate 28 (k = 1), 55 (k = 2), 62 (k = 4); the three registered stock births of this recipe (seeds 5 to 7) gate 65 / 64 / 61 at the same horizon. The k = 4 reading says that half of the 19M model as a fixed random feature map still supports a gate inside the stock range on one seed; the k = 1 and k = 2 readings say the top one to two blocks alone over a random stack do not.
  Honest breaks: one seed, no paired same-seed full-BP arm (the stock births are different seeds), so "inside the stock range" is descriptive, not a matched-function reading at the 120 gate's sigma; the frozen controls were born as controls for a DFA question and their recipe was not tuned for a frozen backbone; the k = 4 v k = 2 step (7 solves) is at the resolution boundary; nothing here is a mechanism claim.
  Testable residue (candidate rung, NOT armed, needs its own pre-reg and Artin GO): FROZEN-BACKBONE-1, fresh paired seeds, full BP v bottom-half-frozen BP (emb + blocks 0..3 frozen at W_0), same W_0 / stream / driver / gate / device; bars on the paired gate difference against the same-writer envelope (n >= 3 pairs by the resolution law); if the frozen arm sits inside the band, the bottom half of the stack is a substitutable random feature map for this task at 19M and the trajectory / dependence desks of the writer program can be pointed at the top half. Relation: a "how much of the stack must be learned" question dual to the credit-anchor frontier's "how much exact credit is needed"; complements the ATOM-DIET and writer-invariance banks.
  MEASURED (2026-09-10, VERDICT FROZEN-BACKBONE-1 RESULTS L70024): the residue ran as PRE-REG FROZEN-BACKBONE-1 (L69697) on seeds 24 / 25 / 26 and REPLICATES: FROZEN 63 / 61 / 59 v same-W_0 FULL 61 / 64 / 62, delta +2 / -3 / -3, every pair inside the -7 law, controls adequate. The 'one seed, no paired arm' break above is closed; what is now open: whether the top four blocks trained over the frozen backbone are the cleanest first arena for the synthetic-gradient writer (Artin direction 2026-09-10, assessment pending), and the untested dependence of the trained top on the specific random features (the 'irrelevant lower stack' reading remains explicitly unclaimed).

- **BANKED (2026-09-14): TASK-GRADIENT GEOMETRY — successful BP training may not need one unique trajectory, but task-loss gradients may occupy a stable, much thinner geometry inside the ambient parameter space** (Artin and an external assistant: the frame, by analogy with measuring geometric distortion in a market-state space; house: the loss-gradient-geometry census instrument, the raw / centered split, the held-out capture law and the ladder).
  The mapping: rows = minibatch task gradients at a frozen state; the object is their empirical geometry (Gram spectrum, held-out capture of a FIT-derived top-k subspace, writer overlap, W_0 baseline), never a Hessian, Fisher, natural gradient, manifold or intrinsic dimension of the loss surface.
  Measured anchors (VERDICT UPDATE-GEOMETRY-CENSUS-0, RESULTS L73446): on the seed-2 forward / backward pair the top-8 FIT subspace reproduces on held batches (S_8 0.39 / 0.40) but captures only 0.25 / 0.27 of a held gradient at k = 8 (0.37 at k = 16), five orders above random: PARTIAL-THIN, not STABLE-THIN; the writers' subspaces on the same batches overlap 0.645 (same-writer rerun pair 0.88): WRITER-SHARED; W_0 is one shared descent direction (C_8 0.94) that training diffuses to a participation ratio of 14; the finals' thinness is an embedding / head / norm reading (77 to 80 % of gradient energy), blocks read 0.005 to 0.03; checkpoint velocity lies almost entirely outside the top-16 gradient subspace (own fraction <= 0.002).
  Honest breaks: one writer pair, one W_0, one probe distribution at batch size 32, 32 + 32 batches (every subspace <= 32-dimensional); the writer overlap is read on the same batches by design; velocity is descriptive (unread AdamW moments, hundreds of steps per interval); a market analogue is a separate object (reflexive, not a fixed loss).
  Testable residue (banked, unarmed, each a new pre-reg and GO): PROJECTED-BP-CAUSAL-1 only if a stable reusable thin subspace fires under a registered target (the OUTSIDE-weighted structure would have to be named explicitly; the global reading did not fire); DATA-LOCUS-GEOMETRY-1 on the stock-v-ATOM paired evidence (instrument validated); FAILED-WRITER-PROJECTION-DESK-0 (descriptive); the cybernetics.bio market-state demo stays outside the lab's experiments. MEASURED 2026-09-14 (VERDICT OPTIMIZER-GEOMETRY-DESK-0, RESULTS L74103): the AdamW writer map applied to the same probe gradients at three stored states reads GEOMETRY-NOT-RESOLVED under the sealed k = 8 gate because the counterfactual write is one common direction per state, the carried first moment (u_0 = 85 to 96 % of the write's norm), with a near-random batch-sensitive remainder (b: held C_8 0.01 to 0.08, reliability 0.06 to 0.16); the deformation of batch geometry enters at temporal integration (D(g -> mhat) 1.9 to 3.9), not at the preconditioner; the writers' write directions are unrelated across writers (S_8 0.002 to 0.005). Reading: at this scale Adam's write coherence is optimizer memory, not compression of the current gradient. Residue (banked, unarmed): OPTIMIZER-MEMORY-ABLATION-1 (same gradient stream, reset or cross-fostered moments) is the causal test of whether that memory direction controls function or path; DESIGNED 2026-09-14 (PRE-REG OPTIMIZER-MEMORY-ABLATION-1, RESULTS L74310, nothing armed): first-moment erasure only (exp_avg <- 0 at the stored step-7200 state of each writer, exp_avg_sq / step counter / weights / scheduler / future batch order native), a 1 % same-axis epsilon twin as the sensitivity reference, CPU-deterministic paired legs of 900 steps, a disjoint mps-calibrated resume-control endpoint precondition against the booked m008100, path label from n_Z(900) (FORGOTTEN <= 0.05 / PERSISTENT >= 0.25) and function label from the HELD-32 CE; v reset, full reset, cross-writer swaps and repeated erasure are excluded from this rung and banked only on a reason. Stateful-writer notation (W_{t+1}, O_{t+1}) = U(W_t, O_t, D_t) adopted prospectively (framing). MEASURED 2026-09-14 (VERDICT OPTIMIZER-MEMORY-ABLATION-1-STAGE-0, RESULTS L74856): NOT-ADJUDICABLE as sealed; writer A's native continuation reproduces the booked 900-step leg to 8e-5 of its norm on CPU and on mps, writer B's two independent native mps resumes from the same stored state and stream diverge by 0.22 of the leg from each other and 0.26 / 0.21 from the booked state (cap 0.25) at held CE unchanged to 3e-5: substrate-level numerical sensitivity of that registered continuation in parameter space, source and scheduler cause not established. The two-writer Stage 1 is closed and no first-moment treatment ran under OMA1; the family stays NOT-ADJUDICABLE, not partially passed. Residue now: (i) PRE-REG FIRST-MOMENT-ERASURE-1 (single writer A at the validated A@7200 arena against the pinned Stage-0 control; RESULTS L74943; not a rescue of OMA1; unarmed, separate GO); (ii) SCHEDULE-PHASE-SENSITIVITY-CENSUS-0 (banked, unarmed): native mps resume pairs across ALL stored milestones of both writers that have a following milestone, rho of each resume to the booked next milestone, pre-registered before any anchor is read, never a favorable single anchor picked after this result; (iii) a longer-horizon or cross-fostered memory leg only on states whose native continuation is reproducible under the like-with-like envelope. MEASURED 2026-09-15 (VERDICT FIRST-MOMENT-ERASURE-1, RESULTS L75193): on writer A at 7200, selective first-moment erasure is exactly the analytic first-step contrast (n_Z(1) = 0.8943 to 1e-8), propagates linearly with the 1 % twin through h = 5 (r 1.00) and h = 20 (0.99), never decays below 0.23 of the control's displacement, ends at 0.45 of the 900-step leg, and leaves the HELD-32 CE unchanged (dCE_Z -0.0013; gate 53 v 56 inside its fence): PERSISTENT-SPECIFIC + FUNCTION-NEUTRAL, path not function, on this writer and anchor. The h = 900 separation is NOT attributable to the erased content specifically (the twin's deviation grows faster than Z's between h = 100 and 900, the two deviations end nearly orthogonal with the same group-share profile): a magnitude-independent late growth mode of the leg carries both arms. Residue re-banked, unarmed: (a) a longer-horizon A leg (7200 -> 15420) WITH a magnitude ladder of twins (1e-3 / 1e-2 / 1e-1) so persistence can be separated from generic late growth; PRE-REGISTERED 2026-09-15 as FIRST-MOMENT-ERASURE-LADDER-1 (RESULTS L75312, docs/preregs/first-moment-erasure-ladder-1.json; design only, unarmed: epsilon in 1e-3 / 1e-2 / 1e-1 / 1 plus C, 12-point grid to H = 8220, regime bars on alpha(H) and cosmin(H), function a separate axis, uniform continuous rerun with in-line qualification against the locked Stage-0 / FME1 digests); RUN 2026-09-15 and NOT-ADJUDICABLE (VERDICT L75795): the frozen one-step preflight refused on eps = 1e-3 (R 1.00126 v 1e-3, cos 0.99869 v 0.999; the three larger arms on the law), no long leg ran, no regime exists; residue re-banked unarmed: the same ladder re-registered with the smallest arm at the resolved scale (1e-2 / 3e-2 / 1e-1 / 1, or a PROSPECTIVE three-point ladder), or a float64-master variant with its own qualification if the 1e-3 arm is wanted; PRE-REGISTERED 2026-09-15 as FIRST-MOMENT-ERASURE-LADDER-2 (RESULTS L75924, docs/preregs/first-moment-erasure-ladder-2.json; design only, unarmed: eps 1e-2 / 1e-1 / 1 + C, same arena and grid, both local decade slopes required for any magnitude-scaled claim, same frozen preflight, no float64 under this GO); MEASURED 2026-09-15 (VERDICT FIRST-MOMENT-ERASURE-LADDER-2, RESULTS L76340): INTERMEDIATE + FUNCTION-NEUTRAL on writer A over eps 1e-2 / 1e-1 / 1: linear and direction-shared through h = 100, decorrelated and curved by h = 900, then settled (magnitude from the epoch boundary, direction once the schedule has cooled) to H = 8220 at n 0.17 / 0.30 / 0.47, decade slopes 0.25 / 0.19, cosmin 0.19 (the TRAJECTORY-SENSITIVE cell missed by 0.020 on alpha_global alone); function unchanged in every arm (|dCE| <= 4.5e-5 at H). The late separation is magnitude-insensitive and direction-decorrelated along the registered axis; whether it is specific to that axis at all needs the matched-norm random-direction control, banked unarmed; the schedule-phase census stays banked; PRE-REGISTERED 2026-09-15 as RANDOM-DIRECTION-CONTROL-1 (RESULTS L76553, docs/preregs/random-direction-control-1.json; design only, unarmed): three seeded random directions built in first-step write space through the exact K map (write norm and 9-group locus matched to the booked FMEL2 eps = 1e-1 arm, cosine to it <= 0.05), fresh C, the booked eps = 1e-1 snapshots as locked comparison vectors, amplification ratio A_r(H) = G_r / G_M with decade bands [1/2, 2] / <= 1/4 / >= 4 for DIRECTION-GENERIC / MOMENT-SPECIFIC / RANDOM-DOMINANT; AMENDED 2026-09-17 (L76847: the draw moved to exp_avg space and matched under the exact write metric, no division by K; REFUTED-IF narrowed to late-amplification specificity) and MEASURED 2026-09-17 (VERDICT RANDOM-DIRECTION-CONTROL-1, RESULTS L77202): MIXED + DIRECTION-INDEPENDENT-LATE + FUNCTION-NEUTRAL; the three matched random directions amplify 195x / 286x / 210x by H against the moment axis's 517x (A_r(H) 0.376 / 0.552 / 0.406), reach 10x at h = 900 against the moment axis's h = 100, end at cos 0.18 to 0.24 to the moment-axis deviation with the same late anatomical profile, and leave the CE unchanged. Reading, fenced: late amplification on this arena is largely direction-generic in magnitude (within 2.7x) with a consistent moment-axis excess (1.8x to 2.7x) and an earlier onset; neither REFUTED-IF fired; FME1's initial-branch claim untouched; the random exp_avg perturbations are 0.17 of the moment-axis one in state norm (descriptive, not matched). ARTIN DECISION 2026-09-17 (AMENDMENT -CONSEQUENCE, RESULTS L77495): no reclassification and no REFUTED-IF amendment, the verdict preserved exactly; scoped synthesis of record: on this writer-A arena large late trajectory amplification is not unique to the carried first-moment direction (all three write/locus-matched random optimizer-state perturbations amplify roughly 195 to 286x; the moment-axis perturbation amplifies earlier and reaches 517x; with the random interventions at about 0.17x the raw exp_avg displacement norm, the excess is not attributable to direction alone); next a zero-training Gram desk (PRE-REG PERTURBATION-RESPONSE-GRAM-DESK-0, RESULTS L77531: normalized deviation Grams and spectra at all 12 horizons and over the 11 windows, global and per group, lagged cross-cosines of the moment axis's 300 -> 900 window against the panel's 900 -> 1800 window with adjacent alternatives; descriptive, no bar) MEASURED 2026-09-17 (OBSERVATION PERTURBATION-RESPONSE-GRAM-DESK-0, RESULTS L77655, zero training): the four deviations are independent through h = 300 and acquire one shared component from h = 900 (top / trace 0.431 at H, participation rank 3.36, depth-uniform); increments are window-local for every arm (lagged cross-cosines |cos| <= 0.09, the PRIMARY lag cell 0.014 / -0.010 / 0.002), so the panel does NOT reproduce the moment axis's growth direction one horizon late; same-window coupling is sign-indefinite in the 300 -> 900 window (-0.34 / -0.38 / +0.28 to M; R1 | R2 0.64) and uniformly positive from 1800 on. Reading (descriptive): the shared late response correlates with window position (the byte-identical stream and schedule phase), not with the perturbation's content; house recommendation, an Artin decision, a perturbation-phase census before any cross-foster; keep-set proposal (FMEL2 e1 / e1e-2 first) recorded, nothing pruned; nothing launched; (b) OPTIMIZER-MEMORY-CROSSFOSTER-1 restricted to reproducible states; (c) the schedule-phase sensitivity census as before. ARTIN DECISION 2026-09-14: OPTIMIZER-GEOMETRY-DESK-0 stays as booked; no post-hoc k = 1 rerun; its k = 1 readouts are citable descriptively only; the low S_8 is compatible with a near-rank-1 write (one stable direction plus seven unstable residual directions depresses a normalized 8-D projector overlap mechanically). PRECONDITIONER-CROSS-FOSTER-1 and UPDATE-SUBSPACE-CAUSAL-1 are not banked (the preconditioner carried little of the deformation; b is not thin).
