...
| "Predicted syndromes will matter more for code and general domains" (mid-null, correctly re-aimed) | the revive-if clause: syndrome prediction pays where the oracle costs seconds (compile+run), not ms — partial recall covers the discount; here the ms-oracle + semantic bits made it a clean NULL (the rules are their own features) | banked with the null, RESULTS 2026-07-13 |
