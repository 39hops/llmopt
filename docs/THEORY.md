...
| MoE routers are DOMAIN-ORGANIZED at deployment: per-domain expert coalitions differ beyond within-domain resampling noise, coalition distance tracks corpus distance — but coverage/recall of demanded experts does NOT predict the SIGN of masking's capability effect (the matched-instrument dissociation), and the beats-full "crest" of masked routing is domain-specific (math 7/7 seeds; mechanics fails under BOTH mask recipes, -59 pooled at the matched recipe) — so coalition STRUCTURE is general, the removable-interference effect is not, and what the excluded experts COMPUTE is the live variable | Expert specialization in MoE (Jacobs et al. 1991 adaptive mixtures; Shazeer et al. 2017; ST-MoE, Zoph et al. 2022 — encoder experts specialize by token type); domain/task expert clustering observed in modern MoEs (OLMoE, Muennighoff et al. 2024; DeepSeekMoE, Dai et al. 2024); pruning-by-frequency as a known-lossy importance proxy (OBD lineage, LeCun et al. 1990) | [ORIGINAL: MOE-GT-2 + GT2-EXCLUSION + D4-PHYS-B (2026-08-04) — decode-only gate-only coalition Jaccard math-phys 0.801 vs math-code 0.533 (prompt-tail-corrected; same-filter split-half nulls 0.921/0.867/0.636), ordering REGISTERED from corpus token overlap (0.329 vs 0.097) before the runs; cross-masks at ~78-80% coverage score 19-21/120 vs random-45%'s 0/120 (R6) and own-crest 76-87 (R4/R5); crest transport to mechanics: decode-recipe +3 (underpowered), matched pooled-recipe -59 pooled 3/3 with open recall 0.9013 vs math's 0.9014 and closed 0.884 vs 0.887 — identical readouts, opposite sign. Fences: one seed per domain, Jaccards are tie-fill upper bounds, functional-vs-topical UNRESOLVED (diagnose-checker artifact, GT2-REVIEW). WIDENED 2026-08-05 (MOE-GT-3/-4/-4b, one seed each): the deepest measured split is REGISTER (symbolic vs verbal), each branch with its own base class — symbolic core 37.1/58 3-way (proofs contain 0.90, prose 0.25), verbal core 48.5/58 2-way vs 26.3 null, cores near-disjoint (J 0.095), cross-branch J pinned 0.16-0.24, shared-SYSTEM confound priced at 0.05-0.10 J (4b) — 'domains' are extensions under the symbolic branch; published-lineage citation for register-level routing separation NOT yet found (gap open — widened clause PROVISIONAL under the no-row-without-citation rule until closed). SHOULDER 2026-08-05 eve (MOE-GT-5/-5b/-5c): branch structure does NOT transfer to a keep rule — union-of-bases at 61.1% keep costs 17.3 solves pooled (n=3 paired seeds, 3/3 negative, above its registered band), and matched-size RANDOM fills over the frozen core score 51/36/48 vs the verbal fill's 55 at equal-or-higher recall — above the core the capability floor is RECALL-driven, fill identity unresolved; free-routing twin: math sends 2.7% of decode demand through verbal-only experts. 'What the excluded experts compute' narrows: their CLASS is not the answer above the core; their demand MASS is] |
