...
| Image | Claim / fence |
|---|---|
| `neurons-19m.png`, `neurons-19m-zoom.png` `[H]` | the first crystal (former README hero) + the whisper zoom. VEHICLE CORRECTION (2026-08-08, grok-seat audit chain): the ZOOM is the Qwen-0.5B run-2b GRPO climb (in-image header "L14 gate" — the 19M has 8 layers), NOT the 19M crystal; the "19m" filename prefix is a rename-era misnomer, kept because [H] files stay frozen. Its booked numbers are the weight-anatomy entry's: the 2.4x verified climb wrote dW 4.0 v 61-87 for SFT (~6% of one SFT run's movement), CKA(pre,climbed) 0.9998 everywhere but the last layer. The mean-displacement figures previously here (0.123/0.007/19x) were never booked and are removed. FROZEN as first rendered |
