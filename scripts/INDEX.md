...
### scripts/render_anim.py
Render driver for the animation triplets (banked repair, 2026-08-13).

- `run(cmd: list[str], env: dict | None=None) -> None`
- `build(stem: str, mode: str) -> None`
- `main() -> None`
...
FIRST-MOMENT-ERASURE-LADDER-2 instrument (PRE-REG RESULTS L75924): the resolved-scale ladder. Writer A at the validated A@7200 arena, FOUR fresh continuous CPU-deterministic legs 7201..15420: the native control C and exp_avg <- (1 - eps) exp_avg for eps in {1e-2, 1e-1, 1} (the amplitudes the FMEL1 target preflight resolved with large margin). Writer A only; no writer-B code path. FMEL1 (VERDICT L75795) is untouched and permanently NOT-ADJUDICABLE; this instrument only READS its locked receipt.

- `finite(x)`
- `alpha_regression(eps_of, ln)` — Independent least-squares slope of ln||dW|| against ln eps over the arms in `ln` (all defined); None with fewer than two points.
- `decade_slope(ln, hi, lo)` — log10(||dW_hi|| / ||dW_lo||) from the natural logs; None if either is UNDEFINED.
- `horizon_metrics(W0, WC, W, eps_of=None, strict_identity=False)` — Per-horizon readouts for the three-point ladder from flats: W0 anchor, WC control, W = {arm: flat}. Zero-norm and non-finite
- `preflight_law(m1, n_pred=N_PRED)` — BAR 1 on the one-step metrics: the registered law unchanged. The receipt separates construction identities (R_1, cos(e1, e1))
- `in_band(x)`
- `regime(mH, preflight_ok)` — BAR 2 at H, first match wins: FORGOTTEN / REGIME-UNRESOLVED / MAGNITUDE-SCALED PERSISTENT / NONLINEAR DIRECTION-SHARED /
...
### scratch/grf_rider3.py
GRF horizon-window rider (desk, frozen pre-run): windowed adjudication of the matched-horizon reading in rider2.json.

- `sha(p)`
- `load_meta(path)`
- `decode_index(traj_path)` — prompt -> list of decode rows, each tagged with the rank of
- `window_table(rows_by, order, cohort, meta)`
- `main()`
...
MATH-CYBER-1 CLOSED-LOOP-1 JOINT-THREE-ARM-RUN-0 — execute the frozen CLOSED-LOOP-1 experiment (prereg 082a.. entry MATH-CYBER-1-CLOSED-LOOP-1-PREREG-0, prereg commit 082d4bc6ed32207a0d0b1fd5d85a56af0a9e4caf) on the sealed 96-root population (manifest sha 50c05794...).

- `fsha(p) -> str`
- `ssha(s: str) -> str`
- `rebuild_root(row)` — Root identity: sympify(sstr) is NOT an identity (reparse
- `rng_choice(root_sha, depth, k)` — Frozen C RNG law (execution-order-independent).
- `b_score_decision(model, dev, parent, acts)` — Arm B one decision: programs + factor codes + total-lp
...
MATH-CYBER-1 REGRET-WALLLIFT-0 (PRE-REG booked at d30e84f4). Closure re-read of the three REGRET-LDS-DESK-0 wall-censored roots under a lifted 3600 s emergency wall, with world successor-materialization time and theta_0 scoring time accumulated SEPARATELY (their sum is the charged wall the kill checks). Every search law preserved from the parent (mathworld1_regret.py): Rank-weighted limited-discrepancy GRAPH search over the realized world snapshot: edge cost = rank-1 (rank-only, never raw scores; no cross-parent score comparison), frontier popped by (path_discrepancy, path_depth, State.key()), TERMINAL-FIRST hard override at expansion (terminal edges cost 0 and carry rank tag "T"), depth cap 12 (nodes at depth 12 never expand), transposition table on full State.key() with the registered strictly-lower-cost reopen law, overflow-unscorable states book state-censored (search continues), 3600 s emergency wall per episode, unique-expansion ladder 12/24/48/96 with primary read at 96. Zero training; frozen theta_0 ranks siblings only.
...
MATH-CYBER-1 SVP-GRID-BIRTH-16001 — the FIRST production birth of the revised IN-SUPPORT-CONFIRMATORY protocol (not a replication claim): the frozen PAIRED FACTOR/HASH production birth over the GRID-DIET combined population (74,860 rows = 73,324 natural + 1,536 balanced covered-grid augmentation), one target-blind batch plan (7,020 updates), one frozen optimization law, ONE bit-identical shared vocab-340 init (seed 16001). ZERO evaluation scoring; this file has no path or string dependency on any evaluation artifact — the frozen IN-SUPPORT eval shas (calibration / sealed heldout / secondary robustness) appear ONLY as opaque provenance literals copied into the receipt (EVAL_PROVENANCE below), never as paths, and nothing here opens, parses, counts, or scores them. Successor fix folded (BIRTH-15001 disclosed deviation): the shared init is materialized by a DEDICATED PRE-STEP (SVPGB16_MAKE_INIT=1, which builds+gates+writes the init and exits); smoke and production REFUSE if the init is absent, so smoke never writes any production path.
...
MATH-CYBER-1 SVP-GRID-BIRTH-17001 — the seed-17001 INIT-ONLY replication birth under the frozen law of REPLICATION-17001-PREREG-0 (cd6db7e3): the frozen PAIRED FACTOR/HASH production birth over the GRID-DIET combined population (74,860 rows = 73,324 natural + 1,536 balanced covered-grid augmentation), one target-blind batch plan (7,020 updates), one frozen optimization law, ONE bit-identical shared vocab-340 init (seed 17001). ZERO evaluation scoring; this file has no path or string dependency on any evaluation artifact — the frozen IN-SUPPORT eval shas (calibration / sealed heldout / secondary robustness) appear ONLY as opaque provenance literals copied into the receipt (EVAL_PROVENANCE below), never as paths, and nothing here opens, parses, counts, or scores them. Successor fix folded (BIRTH-15001 disclosed deviation): the shared init is materialized by a DEDICATED PRE-STEP (SVPGB17_MAKE_INIT=1, which builds+gates+writes the init and exits); smoke and production REFUSE if the init is absent, so smoke never writes any production path.
...
MATH-CYBER-1 SVP-GRID-P2-MATERIALIZATION-QUALIFICATION-0 — the FIRST and FINAL realization of HASH-P2 under the frozen law of PERMUTATION-REPLICATION-PREREG-0 (commit 7976613b3fef18f54d953ac0404f377e5c74031b). ONE invocation: implement the salt-changed Feistel exactly as frozen, qualify it exhaustively, record the preregistered descriptive realization census, write the receipt. NO model initialization, training, checkpoint creation, calibration/heldout scoring, or model inference anywhere in this file.

- `_round2(i, x)`
- `feistel2(v, inverse=False)`
- `hash2_symbols(rule, sk, so, pk, pi)`
- `hash2_decode(sym)`
- `fsha(p) -> str`
- `main()`
...
- `e8m0(Wb)`
- `nearest(Wn, lv)`
- `assign(V, C, chunk=1 << 18)`
- `kmeans(Vn, seed)`
- `stack_train(Vn, stages, tag)`
- `vq(Vv, st)`
